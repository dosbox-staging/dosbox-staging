...
sudo apt-get install git build-essential pkg-config cmake curl ninja-build \
             autoconf bison libtool libgl1-mesa-dev
```

TODO: Try once again, with fresh install - might be missing _libopengl-dev_
...
3. Clone the DOSBox Staging into your home directory:
...
4. Configure the sources and build the DOSBox:
...
5. You can now launch DOSBox with command:

```bash
./build/release-linux/dosbox
```

TODO: It must be missing more dependencies, DOSBox now bails out with:

```
ABORT: SDL: Can't init SDL dsp: No such audio device
```
