...
protected:
	...
	void SetUp() override
	{
		...
		// Printer output now lands under capture_dir. Point capture
		// at the per-test tmpdir and initialise it so the printer
		// writes there when it asks capture for a file.
	...
	// always print0001.png.
	std_fs::path render(const std::vector<uint8_t>& bytes)
	{
		return render_with_pins(bytes, 24);
	}

	// Same as render() but with a configurable pin count for tests
	// that exercise the 9-pin branches in the dispatch.
	...
	// Used by the escapy-ported tests below where a larger page is
	// needed to match escapy's layout.
	                              ...
	                              const double height_in, const int pins = 24)
	{
		...
		// dir and starts at print0001.png again. Tests call render()
		// multiple times per test and read print0001.png each time.
		CAPTURE_Destroy();
		CAPTURE_Init();
...
TEST_F(PrinterRenderTest, Text_DoubleWidthHeight_9pin_Escapy)
{
	...
	append_crlf_joined(
	        bytes,
	        {
	                ...
	                cat({point_8,
	                     b("NOTE: In 9pins mode, double-height should temporarily "
	                       "stop upper/subscripting, condensed font and Draft "
	                       "printing.")}),
	                ...
	                cat({point_8,
	                     b("upperscripting should have no effect in 9pins mode"),
	                     reset_intercharacter_space}),
	                cat({enable_upperscripting,
	                     double_height,
	                     pangram,
	                ...
	                cat({point_8,
	                     b("in 9pins mode make sure scripting is enabled, then "
	                       "disabled by double-height, then disabled, then not "
	                       "set anymore when exiting double-height"),
	                     reset_intercharacter_space}),
	                cat({enable_upperscripting,
	                     b("The quick "),
	                     double_height,
	                ...
	                cat({point_8,
	                     b("upperscripting should have no effect on the first "
	                       "part in 9pins mode"),
	                     reset_intercharacter_space}),
	                cat({double_height,
	                     enable_upperscripting,
...
TEST_F(PrinterRenderTest, Text_ControlCodesPrinting_9pin_Escapy)
{
	...
	// SELECTED control-code set (escapy PrintControlCodes.SELECTED),
	// sorted ascending.
	...
	std::vector<uint8_t> bytes;
	bytes.insert(bytes.end(), {0x1B, '@'}); // esc_reset prefix
...
protected:
	...
	void SetUp() override
	{
		...
		PRINTER_Configure(PrinterModel::PostScript, 0, 0.0, 0.0, 0);
	}
...
protected:
	...
	void SetUp() override
	{
		...
		PRINTER_Configure(PrinterModel::Passthrough, 0, 0.0, 0.0, 0);
	}
...
{
	// Simulated PCL Transfer Raster Data payload: arbitrary 8-bit
	// bytes including 0x0C (the form-feed character) appearing as
	// raster data, NOT as a page-eject control code.
...
protected:
	...
	{
		// Control register bits used here (see printer.cpp).
...
TEST_F(PrinterPostScriptRegulationTest, FormFeedDoesNotCloseFile)
{
	...
	// User presses Ctrl+F2 (or the timeout fires). In PS mode this is
	// a no-op — the in-flight file stays open.
	PRINTER_FormFeed(true);

	// Continue the job after the would-be eject — these bytes must
	// land in the SAME file as the first half.
	for (const char c : std::string(" more")) {
		strobe_byte(static_cast<uint8_t>(c));
	}

	// End-of-job ^D — this is what closes the file.
	strobe_byte(0x04);

	const auto out_path = output_dir / "print0001.ps";
	ASSERT_TRUE(std_fs::exists(out_path));

	// No second file should have appeared.
	EXPECT_FALSE(std_fs::exists(output_dir / "print0002.ps"));

	// File contents are the full byte stream minus the trailing ^D.
