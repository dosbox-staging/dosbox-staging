...
layout(location = 1) out vec2 prescale;
...
layout(location = 1) in vec2 prescale;
...
vec4 texture_linear(in sampler2D tex, in vec2 uv)
{
	// subtract 0.5 here and add it again after the floor to centre the texel
	vec2 texCoord = uv * INPUT_SIZE_0 - vec2(0.5);
	...
	vec4 c_s0t0 = GAMMA_IN(texture(tex, s0t0 * invTexSize));
	vec4 c_s0t1 = GAMMA_IN(texture(tex, s0t1 * invTexSize));
	vec4 c_s1t0 = GAMMA_IN(texture(tex, s1t0 * invTexSize));
	vec4 c_s1t1 = GAMMA_IN(texture(tex, s1t1 * invTexSize));
...
void main() {
	...
	vec2 mod_texel = min(texel_floored + f, INPUT_SIZE_0 - halfp);

	FragColor = GAMMA_OUT(texture_linear(INPUT_TEXTURE_0, mod_texel / INPUT_SIZE_0));
}
