...
To start a new translation, execute the following commands inside the current
alpha build of DOSBox Staging:
...
where _xx_ is a language code. If only possible, use a two-letter code
according to the [ISO 639](https://en.wikipedia.org/wiki/List_of_ISO_639_language_codes)
standard.
...
Before you start editing any current `PO` file, you need to refresh its
...
marked accordingly, and without messages that were removed from the source
code. This newly created file is ready for updating the translation.
...
The file consists of entries of roughly the same layout, separated with empty
...
always contains an empty English string (`msgid`); all the metadata entries are
put in a translated string (`msgstr`).
...
DOSBox Staging needs some additional, non-standard metadata — it can be
recognized by `#METADATA` keyword present in both the context (`msgctxt`) and
location (`#:`) fields, for example:

```po
#. Do not translate this, set according to the instruction!
#: #METADATA
#, fuzzy
msgctxt "#METADATA"
msgid ""
...
target language; it is going to be used to determine which DOS code pages are
...
The `msgstr` contains a translated message, empty if the translation is missing.
This is the part you need to update :)

Last but not least, the `fuzzy` flag we have already seen — it is set by DOSBox
Staging to tell you this particular entry needs your attention. This might be
a missing translation, an outdated translation (it is detected by comparing
`msgid` to the current English message in the code), or any other reason — see
the warning logs when loading the translation. After updating the translation,
remove it!
...
Similarly, all the quotation symbols, `"`, have to be replaced with _escape
sequence_ `\"`.
...
C/C++ programming languages; they are replaced with another string or number,
using the specified format.

You might need to understand the notation to properly format the output (like tables,
when the translated headers have different lengths than in English). Search the
web to learn more about it; here are a few links:
...
to use the translation for this particular message, prints out a warning log,
...
Now you need to clean up the file. Once again start DOSBox Staging (this time
with your updated `PO` file) and once again execute the commands:
...
formatting style. It is especially important if you used a dedicated `PO` file
editor.
...
After these steps are done, and after you have tested your translation (see the
next chapter for some tips), you can submit the updated translation for
inclusion into DOSBox Staging.
