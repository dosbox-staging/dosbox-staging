        ...
        if (p_mp3->p_dr != NULL) {
            ...
            if (result == DRMP3_TRUE) {
                ...
                if (num_frames != 0) {
                    ...
                    // BAIL_MACRO("MP3: populate_seek_table failed to create seek points for the stream; falling back to brute-force seeking.", 0);
                }
            } // else { BAIL_MACRO("MP3: drmp3_init(...) failed to parse and initialize the mp3 stream", 0); }
        } // else { BAIL_MACRO("MP3: failed to allocate memory for the drmp3 object", 0); }
    } // else { BAIL_MACRO("MP3: failed to allocate memory for the mp3_t object", 0); }
