...
bool Prop_int::ValidateValue(const Value& in)
{
	...
	NOTIFY_DisplayWarning(Notification::Source::Console,
	                      ...
	                      "Invalid [color=light-green]'%s'[reset] setting: "
	                      "[color=white]'%s'[reset]. "
	                      "Value is outside of the valid range of %s-%s, "
	                      "using [color=white]'%d'[reset]",
	                      propname.c_str(),
	                      in.ToString().c_str(),
...
bool Prop_bool::SetValue(const std::string& input)
{
	...
	if (!is_valid) {
		...
		// TODO duplicate?
		NOTIFY_DisplayWarning(Notification::Source::Console, "CONFIG",
...
bool Section_prop::HandleInputline(const std::string& line)
{
	...
	for (auto& p : properties) {
		...
		if (p->IsDeprecated()) {
			// TODO is this message correct? setting instead of option?
			NOTIFY_DisplayWarning(Notification::Source::Console,
			                      "CONFIG",
			                      "Deprecated option "
								  "[color=light-green]'%s'[color=white]",
			                      name.c_str());
	...
	NOTIFY_DisplayWarning(Notification::Source::Console,
	                      "CONFIG",
	                      "Invalid option [color=light-green]'%s'[reset]",
	                      name.c_str());
	return false;
}
