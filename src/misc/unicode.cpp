...
using box_drawing_set_t = std::array<unicode_code_point, 40>;
...
using alias_t        = std::pair<unicode_code_point, unicode_code_point>;
...
public:
	Grapheme() = default;
	Grapheme(const unicode_code_point code_point);

	[[nodiscard]] bool IsEmpty() const;
	[[nodiscard]] bool IsValid() const;
	[[nodiscard]] bool HasMark() const;
	[[nodiscard]] unicode_code_point GetCodePoint() const;
	void PushInto(unicode_string& str_out) const;
...
private:
	// Unicode code point
	unicode_code_point code_point = static_cast<unicode_code_point>(' ');
	// Combining marks
	unicode_string marks        = {};
	unicode_string marks_sorted = {};
...
using map_box_code_points_t = std::map<unicode_code_point, unicode_code_point>;

// Mapping between lowercase and uppercase code points
using map_code_point_case_t    = std::map<unicode_code_point, unicode_code_point>;
using map_dos_character_case_t = std::vector<uint8_t>;

// Rules how to decompose code points to split-out all the combining marks
using decomposition_rules_t = std::map<unicode_code_point, Grapheme>;
...
{
	// clang-format off
	static constexpr std::pair<unicode_code_point, unicode_code_point> Ranges[] = {
...
Grapheme::Grapheme(const unicode_code_point initial_code_point)
...
unicode_code_point Grapheme::GetCodePoint() const
{
	return code_point;
}

void Grapheme::PushInto(unicode_string& str_out) const
{
...
void Grapheme::AddMark(const unicode_code_point in_code_point)
{
	if (!is_valid) {
...
constexpr std::array<unicode_code_point, 0x20> ScreenCodesWide = {
	// clang-format off
...
constexpr unicode_code_point ScreenCodeWide7f = 0x2302; // 'del' character

static bool is_control_code(const unicode_code_point value)
{
	return (value < ControlCodeThreshold) || (value == ControlCodeDelete);
}

static std::optional<unicode_code_point> control_code_to_unicode(const uint8_t byte,
...
static unicode_string utf8_to_unicode_common(const std::string& str)
{
	...
	for (size_t i = 0; i < str.size(); ++i) {
		...
		str_out.push_back(static_cast<unicode_code_point>(code_point));
	}

	return str_out;
}

static std::string wide_to_utf8(const unicode_string& str)
{
	std::string str_out = {};
...
static void warn_code_point(const unicode_code_point code_point)
{
	static std::set<unicode_code_point> already_warned;
...
static std::string wide_to_dos(const unicode_string& str,
                               const DosStringConvertMode convert_mode,
                               ...
                               const uint16_t code_page)
{
	...
	auto push_unknown = [&](const unicode_code_point code_point) {
...
static unicode_string dos_to_unicode_common(const std::string& str,
                                            const DosStringConvertMode convert_mode,
                                            const uint16_t code_page)
{
	unicode_string str_out = {};
...
{
	unicode_code_point code_point = 0;
...
static void error_code_point_found_twice(const unicode_code_point code_point,
                                         const std::string& file_name,
...
static void import_config_main(const std_fs::path& path_root)
{
	...
	while (get_line(in_file, line_str, line_num)) {
		...
		if (tokens[0] == "ALIAS") {
			...
			unicode_code_point code_point_1 = 0;
			unicode_code_point code_point_2 = 0;
...
static void import_mapping_case(const std_fs::path& path_root)
{
	...
	std::set<unicode_code_point> all_uppercase = {};
	std::set<unicode_code_point> all_lowercase = {};
	...
	while (get_line(in_file, line_str, line_num)) {
		...
		unicode_code_point code_point_upper = 0;
		unicode_code_point code_point_lower = 0;
...
unicode_string dos_to_unicode(const std::string& str,
                              const DosStringConvertMode convert_mode)
{
	load_config_if_needed();
