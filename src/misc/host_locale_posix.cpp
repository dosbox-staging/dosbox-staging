...
static const std::string XdgSessionType    = "XDG_SESSION_TYPE";
...
// several historic cuntries/teritories. Reference:
...
static const std::map<std::string, KeyboardLayoutMaybeCodepage> X11ToDosKeyboard = {
	...
	//   DOS layouts:
	//   bg241, br274, bn, ce, ce443, sx, gk459, gk220, gr453, is, ix, ne,
	//   rx443, tt443, ur465, ur2001, ur2007
...
static std::vector<std::string> get_command_output(const std::string &command)
{
	...
	while(getline(buffer, &buffer_size, file_pointer))
	{
		assert(buffer);
		assert(*buffer);
...
static std::pair<std::string, std::string> get_env_variable_from_list(const std::vector<std::string> &list)
{
...
static bool is_xdg_desktop_session(const XdgDesktopSession session)
{
	...
	// To distinguish between X11 and Wayland sessions
	auto get_xdg_session_type = []() {
		auto variable = get_env_variable(XdgSessionType);
		upcase(variable);
		return variable;
	};
	...
	static const auto xdg_session_type    = get_xdg_session_type();
	...
	static const bool is_x11     = (xdg_session_type == "X11");
	static const bool is_wayland = (xdg_session_type == "WAYLAND");
	...
	// XXX check Wayland version of Raspberry Pi Os

	switch (session) {
	case XdgDesktopSession::X11:     return is_x11;
	case XdgDesktopSession::Wayland: return is_wayland;
...
{
	const std::vector<std::string> Variables = {
		VariableLanguage, LcAll, LcMessages, VariableLang,
	};
...
static std::vector<std::pair<std::string, std::string>> consolidate_layouts_variants(const std::vector<std::string> &layouts,
                                                                                     const std::vector<std::string> &variants)
{
	...
	if (layouts.size() == variants.size()) {
		for (size_t i = 0; i < layouts.size(); ++i) {
			result.push_back({layouts[i], variants[i]});
		}
	} else {
...
static std::vector<std::pair<std::string, std::string>> get_keyboard_layouts_x11()
{
	auto try_using_command = [&](const std::string &command)
		-> std::vector<std::pair<std::string, std::string>> {

		auto command_output = get_command_output(command);

		std::vector<std::string> layouts  = {};
		std::vector<std::string> variants = {};
		for (auto &line : command_output)
		{
			lowcase(line);
			...
			if (tokens[idx_key] == "layout:" &&
				...
				layouts = split(tokens[idx_value], ",");
			} else if (tokens[idx_key] == "variant:") {
				variants = split(tokens[idx_value], ",");
			}
		}

		return consolidate_layouts_variants(layouts, variants);
	};

	// First try to use the standard X11 command
	...
	// It is possible 'setxkbmap' is not installed; if under Linux, we can
	// also try the systemd command
...
static std::vector<std::pair<std::string, std::string>> get_keyboard_layouts_kde()
{
	const std::string SectionStart = "[Layout]";
	const std::string LayoutsKey   = "LayoutList";
	const std::string VariantsKey  = "VariantList";

	std::vector<std::string> layouts  = {};
	std::vector<std::string> variants = {};

	std::ifstream config_stream(get_xdg_config_home() / "kxkbrc");
	std::string input_line = {};
	while (getline(config_stream, input_line))
	{
		trim(input_line);
		if (input_line != SectionStart) {
			continue;
		}
		...
		while (getline(config_stream, input_line))
		{
			...
			if (tokens[0] == LayoutsKey) {
				layouts = split(tokens[1], ",");
			} else if (tokens[0] == VariantsKey) {
...
static std::vector<std::pair<std::string, std::string>> get_keyboard_layouts_gnome()
{
	const std::string Command = "gsettings get org.gnome.desktop.input-sources sources";
	auto command_output = get_command_output(Command);
	...
	auto trim_strip_leading = [](const std::string &input,
	                             const char delimiter_leading) -> std::optional<std::string> {
		std::string tmp = input;
		trim(tmp);

		if (tmp.size() < 1 ||
		    !tmp.starts_with(delimiter_leading)) {
			return {};
		}
	...
	auto trim_strip_both = [](const std::string &input,
	...
	std::string buffer = command_output[0];
	trim(buffer);
...
static std::vector<std::string> get_keyboard_layouts_tty()
{
	...
	while (getline(console_params_stream, input_line))
	{
		...
		trim(tokens[0]);
		trim(tokens[1]);
		// XXX what if more than one KEYMAP keyword is present?
	...
	if (getline(console_params_stream, input_line)) {
...
static std::vector<KeyboardLayoutMaybeCodepage> get_layouts_maybe_codepages_desktop(std::string &log_info)
{
	const std::string SourceX11   = "X11:";
	const std::string SourceKde   = "KDE:";
	const std::string SourceGnome = "GNOME:";

	std::string source_desktop = {};

	log_info = {};

	std::vector<std::pair<std::string, std::string>> results_desktop = {};

	// For X11 sessions, try the generic X11 mechanisms first.
	if (is_xdg_desktop_session(XdgDesktopSession::X11)) {
		results_desktop = get_keyboard_layouts_x11();
		if (!results_desktop.empty()) {
			source_desktop = SourceX11;
		}
	}
	...
	// Also use them in the unlikely event of X11 methods failing.
	if (results_desktop.empty()) {
		...
		} else if (is_xdg_desktop_session(XdgDesktopSession::Gnome)) {
	...
	// On Wayland, try the generic X11 mechanisms as a last resort (they
	// might be unreliable), only accept results if they are non-trivial, as
	// on some environments (at least on KDE 6) they return 'us' layout
	if (results_desktop.empty() && is_xdg_desktop_session(XdgDesktopSession::Wayland)) {
		results_desktop = get_keyboard_layouts_x11();
		if (results_desktop.size() == 1 &&
		    results_desktop[0].first == "us" &&
		    results_desktop[0].second.empty()) {
		    	// Trivial results, do not accept them
		    	return {};
		}
	...
	for (const auto &entry : results_desktop) {
		std::string key1 = {};
		std::string key2 = {};

		if (!entry.second.empty()) {
			// First - try to match layout+variant
			key1 = entry.first + ":" + entry.second;
...
#ifdef __linux__
static std::vector<KeyboardLayoutMaybeCodepage> get_layouts_maybe_codepages_tty(std::string &log_info)
{
	const std::string source_tty = "[TTY] ";
...
#ifdef __linux__
	// Try to get keyboard layouts from the terminal settings
...
HostLocale DetectHostLocale()
{
	...
	if (!locale) {
		...
		locale->keyboard_layout_list = get_layouts_maybe_codepages(log_info.keyboard);
	}
