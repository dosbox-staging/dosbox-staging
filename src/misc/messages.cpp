...
public:
	...
	void Verify(const std::string& name, const Message& english);
...
private:
	...
	MsgFlags message_flags = {};

	// Original text, UTF-8, can contain DOSBox ANSI markups
	std::string text_raw = {};
	// Text in DOS encoding, markups converted to ANSI control codes
	std::string text_dos = {};
...
Message::Message(const MsgFlags flags, const std::string& text)
        : is_english(true),
          message_flags(flags),
          text_raw(text)
{}

Message::Message(const std::string& text) : is_english(false), text_raw(text) {}
...
void Message::Verify(const std::string& name)
{
	assert(is_english);
	if (is_verified) {
		return;
	...
	if (!(message_flags & MsgFlagNoFormatString)) {
...
void Message::Verify(const std::string& name, const Message& english)
{
	assert(!is_english);
	if (is_verified) {
		return;
	...
	message_flags = english.message_flags;
                               ...
                               const bool skip_code_page_check = false)
{
	...
	if ((is_code_page_compatible || skip_code_page_check) &&
		...
		return dictionary_translated.at(name).Get(raw_requested).c_str();
	}
	...
	return dictionary_english.at(name).Get(raw_requested).c_str();
}
...
static bool load_messages(const std_fs::path& file_name)
{
	...
	while (std::getline(in_file, line)) {
		...
		if (dictionary_english.contains(name)) {
			translated.Verify(name, dictionary_english.at(name));
		}
	}
...
static bool save_messages(const std_fs::path& file_name)
{
	...
	for (const auto& name : message_order) {
		...
		std::string text = {};
		if (dictionary_translated.contains(name)) {
			text = dictionary_translated.at(name).Get(RawRequested);
		} else {
			assert(dictionary_english.contains(name));
			text = dictionary_english.at(name).Get(RawRequested);
		}
...
void MSG_Add(const std::string& name, const std::string& text)
{
	MSG_Add(name, 0, text);
}

void MSG_Add(const std::string& name, MsgFlags flags, const std::string& text)
{
	if (dictionary_english.contains(name)) {
		constexpr bool RawRequested = true;

		if (dictionary_english.at(name).Get(RawRequested) != text) {
	...
	dictionary_english.emplace(name, Message(flags, text));

	auto& english = dictionary_english.at(name);
	english.Verify(name);
	if (dictionary_translated.contains(name)) {
		dictionary_translated.at(name).Verify(name, english);
	}
}
