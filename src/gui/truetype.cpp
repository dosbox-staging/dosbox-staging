...
private:
	...
	static constexpr int   MaxStepDelta     = 4;
	static constexpr int   MaxStepStretch   = 4;
	                                     ...
	                                     const FT_Render_Mode render_mode)
{
	...
	// XXX cap the value to prevent going over the cell heigth
	...
	// XXX check if values are sane
        ...
        callibration_tweaked.stretch_y += coefficient * cell.GetSizeY();

        // XXX this formula is wrong; it needs to take into account current delta_y too
        callibration_tweaked.delta_y -= coefficient * (cell.GetSizeY() - cell.GetDistanceTop()) / 2;
...
Cell FontWrap::RenderCellSymbol(const FT_UInt glyph_index,
                                ...
                                const bool is_up_down_arrow)
{
	constexpr auto Flags = FT_RENDER_MODE_NORMAL; // XXX 'Flags' -> 'RenderMode'?

	auto callibration_tweaked = callibration[Category::Symbol];

	const auto cell = RenderCellGeneric(glyph_index, callibration_tweaked, Flags);
	...
	if (preserve_aspect) {
		return RenderCellPreserveAspectRatio(glyph_index, callibration_tweaked, Flags);
	} else {
		return RenderCellGeneric(glyph_index, callibration_tweaked, Flags);
	}
}
                                  ...
                                  const bool should_overlap)
{
	...
	// XXX considering using Embolden X
...
void FontWrap::PreRender(const size_t width, const size_t height)
{
	...
	pixel_aspect_ratio = pixel_aspect_ratio / (640.0f / 400.0f); // XXX get the screen resolution, refresh if changed
