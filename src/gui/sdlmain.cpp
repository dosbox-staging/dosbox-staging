...
{
	const std::string vsync_pref = get_sdl_section()->Get_string("vsync");
	...
	auto parse_setting = [](const std::string& s) -> std::optional<bool> {
		if (has_true(s)) {
			return true;
		}
		if (has_false(s)) {
	...
	const auto parts = split(vsync_pref);
	...
	if (parts.size() == 1) {
		if (const auto mode_opt = parse_setting(parts[0]); mode_opt) {
			sdl.vsync.windowed   = *mode_opt;
			sdl.vsync.fullscreen = *mode_opt;
		} else {
			handle_error();
		}

	} else {
		const auto windowed_opt   = parse_setting(parts[0]);
		const auto fullscreen_opt = parse_setting(parts[1]);

		if (windowed_opt && fullscreen_opt) {
			sdl.vsync.windowed   = *windowed_opt;
			sdl.vsync.fullscreen = *fullscreen_opt;
		} else {
			handle_error();
		}
	}
...
{
	const std::string presentation_mode_pref = get_sdl_section()->Get_string(
	        "presentation_mode");
...
static void setup_presentation_mode()
{
	...
	case PresentationMode::HostRate: {
		...
		// We need to present the frame a bit before vsync happens, otherwise
		// we'll "miss the train" and will need to wait an extra frame period.
		...
		// with the time window rather than miss the frame.
		//
		// This value was determined by experimentation on our supported OSes.
		// We might make this configurable if there's evidence that no single
		// single value can work well on all systems.
		//
                                        ...
                                        const int render_height_px)
{
	...
	auto use_srgb_framebuffer = false;
...
{
	if (sdl.updating) {
		// `sdl.updating` is true when the contents of the framebuffer has been
	...
	if (GFX_GetPresentationMode() == PresentationMode::DosRate) {
		...
		// In 'dos-rate' presentation mode, we're presenting the frames as
		// soon as they arrive. This caters for the VRR monitor use case
		// where effectively our present rate dictates the refresh rate of
		// the monitor.
		//
		// The `GFX_StartUpdate/GFX_EndUpdate` pair is called at regular
		// intervals close to the actual refresh rate of the emulated DOS
		// video mode (`GFX_EndUpdate` alone without a matching
		// `GFX_StartUpdate` call is called more often). There is some
		// jitter in the 1-5 ms range, but this seems to work well enough
		// in practice, at least on some VRR monitors.
		//
		// However, there is room for improvement as the slightly uneven
		// pacing might cause flicker and less than perfect smooth
		// scrolling on some VRR monitor & driver combinations. We should
		// try to get our present rate as close to the DOS rate as
		// possible in the future, down to the microsecond level (e.g., by
		// tightening the timing accuracy of the PIC timers as VGA updates
		// are timed by abusing the emulated PIC timers).
		//
		// But because updating the new texture to the GPU also takes some
		// non-zero time, we'll probably need to introduce an extra fixed
		// latency to account for this delay, and possibly make
		// adjustments in the audio emulation layer to keep the video and
...
void GFX_MaybePresentFrame()
{
	...
	if (force_present || (curr_frame_time_us < max_frame_time_us)) {
		...
		if (force_present || (curr_frame_time_us >= present_window_start_us)) {
			...
			LOG_TRACE("DISPLAY: frame_time_ms: %2.4f", 0.001 * measured_frame_time_us);
#endif
#if 1
...
#if 1
		LOG_ERR("frame_time_us %2.4f, curr %2.4f", 0.001 * sdl.presentation.frame_time_us, 0.001 * curr_frame_time_us);
...
static void init_sdl_config_section()
{
	...
	pstring = sdl_sec->Add_string("vsync", always, "off");
	pstring->Set_help(
	        "Set the host video driver's vertical synchronization (vsync) mode.\n"
	        "You can specify a single vsync setting for both windowed and fullscreen mode\n"
	        "(e.g. `vsync = on`), or two settings where the first one is for windowed mode\n"
	        "and the second one is for fullscreen (e.g., `vsync = off on` to only enable\n"
	        "vsync in fullscreen).\n"
	        "  off:  Disable vsync (default). This is the best option on variable-refresh\n"
	...
	pstring->Set_help(
		...
		"  auto:       Use 'host-rate' if vsync is enabled (see 'vsync'), otherwise use\n"
		"              'dos-rate' (default).\n"
