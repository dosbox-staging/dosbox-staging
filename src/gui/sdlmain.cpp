...
void maybe_log_opengl_error(const char* message)
{
	GLenum r = glGetError();
	if (r == GL_NO_ERROR) {
		return;
...
#else
void maybe_log_opengl_error(const char*)
{
	return;
}
#endif
...
static void handle_sdl_windowevent(const SDL_Event& event)
{
	...
	if (sdl.pause_when_inactive) {
		if ((event.window.event == SDL_WINDOWEVENT_FOCUS_LOST) ||
			...
			// Window has lost focus, pause the emulator. This is
			// similar to what PauseDOSBox() does, but the exit
			// criteria is different. Instead of waiting for the
			// user to hit Alt+Break, we wait for the window to
...
[[maybe_unused]] static void apply_windows_debugger_workaround(const bool is_console_disabled)
{
...
int sdl_main(int argc, char* argv[])
{
	...
	try {
		...
		if (const return_code = maybe_handle_command_line_output_only_actions();
		    return_code) {
			return return_code;
		}
