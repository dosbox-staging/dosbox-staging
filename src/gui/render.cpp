...
static void init_render_settings(SectionProp& secprop)
{
	...
	string_prop->SetHelp(
	        "Set the viewport size ([color=white]'fit'[reset] by default). This is the maximum drawable area;\n"
	        "the video output is always contained within the viewport while taking the\n"
	        "configured aspect ratio into account (see [color=light-green]'aspect'[reset]). Possible values:\n"
	        "  [color=white]fit[reset]             Fit the viewport into the available window/screen (default).\n"
	        "                  There might be padding (black areas) around the image with\n"
	        "                  [color=light-green]'integer_scaling'[reset] enabled.\n"
	        "  [color=white]WxH[reset]             Set a fixed viewport size in [color=white]WxH[reset] format in logical units\n"
	        "                  (e.g., 960x720). The specified size must not be larger than\n"
	        "                  the desktop. If it's larger than the window size, it will be\n"
	        "                  scaled to fit within the window.\n"
	        "  [color=white]N%%[reset]              Similar to [color=white]'WxH'[reset], but the size is specified as a percentage of\n"
	        "                  the desktop size.\n"
	        "  [color=white]relative H%% V%%[reset]  The viewport is set to a 4:3 aspect ratio rectangle fit into\n"
	        "                  the available window or screen, then is scaled by the [color=white]H[reset] and\n"
	        "                  [color=white]V[reset] horizontal and vertical scaling factors (valid range is\n"
	        "                  from 20%% to 300%%). The resulting viewport is allowed to extend\n"
	        "                  beyond the bounds of the window or screen. Useful to force\n"
	        "                  arbitrary display aspect ratios\n"
	        "                  with [color=light-green]'aspect = stretch'[reset] and to \"zoom\" into the image.\n"
	        "                  This effectively emulates the horizontal and vertical\n"
	        "                  stretch controls of CRT monitors.\n"
	        "\n"
	        "Notes:\n"
	        "  - Using [color=white]'relative'[reset] mode with [color=light-green]'integer_scaling'[reset] enabled could lead to\n"
	        "    surprising (but correct) results.\n"
	        "  - You can use the [color=white]'Stretch Axis'[reset], [color=white]'Inc Stretch'[reset], and [color=white]'Dec Stretch'[reset] hotkey\n"
	        "    actions to set the stretch in [color=white]'relative'[reset] mode in real-time.");
