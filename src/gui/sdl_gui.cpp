...
// single pixel), otherwise running the scalers, the deinterlacer (if
// enabled), and submitting the new frame to the render backend can be
// optimised away (common scenario in DOS games with low frame rates and
// mostly static screens, e.g., most RPG, adventure, and strategy titles).
//
// So we have two scenarios:
//
// 1. Current frame is the same as the previous frame:
//
//    - `GFX_StartUpdate()` is NOT called for this frame.
//    - `GFX_EndUpdate()` IS called; `sdl.draw.updating_framebuffer` is FALSE.
//
// 2. Current frame contains changes since the previous frame:
//
