...
static void handle_auto_image_settings(const VideoMode& video_mode)
{
	const auto maybe_auto_settings =
	        AutoImageAdjustmentSettingsManager::GetInstance().GetSettings(
	                video_mode);
...
constexpr auto BrightnessMin = 0;
constexpr auto BrightnessMax = 100;

constexpr auto ContrastMin = 0;
constexpr auto ContrastMax = 100;

constexpr auto GammaMin = -50;
constexpr auto GammaMax = 50;

constexpr auto DigitalContrastMin = -50;
constexpr auto DigitalContrastMax = 50;
...
static void init_render_settings(SectionProp& section)
{
	...
	bool_prop->SetHelp(
	        ...
	        "output. The color space conversion is always active, that cannot be disabled\n"
	        "(see 'color_space').");
	...
	constexpr int DefaultBrightness = 40;
	...
	constexpr int DefaultContrast = 55;
	...
	int_prop->SetHelp(
	        format_str("Set the digital contrast of the video output (%d by default). Valid range is %d to %d.\n"
	                   "Notes: See 'brightness' for further details.",
...
{
	curr_image_adjustment_settings.black_level_color = VGA_GetBlackLevelTint();
}
...
static void update_color_temperature_setting()
{
	if (const auto maybe_color_temperature = get_color_temperature_setting_value();
		...
		curr_image_adjustment_settings.color_temperature_kelvin = *maybe_color_temperature;
	} else {
		curr_image_adjustment_settings.color_temperature_kelvin = ColorTemperatureNeutral;
	}
}
...
static void select_image_setting_control(const Direction dir)
{
	...
	const auto name = [] {
		...
		case ColorTemperatureLumaPreserve:
			return "color temperature luma preserve";
			break;
...
static void adjust_image_setting(const Direction dir)
{
	...
	case ColorTemperature: {
		...
		const auto curr_value = maybe_color_temperature
		                              ? *maybe_color_temperature
