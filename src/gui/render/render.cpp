...
static void latch_last_complete_source()
{
	...
	const auto first = static_cast<size_t>(first_row);
	const auto last  = static_cast<size_t>(last_row);

	const auto offset_bytes = first * pitch;
	const auto num_bytes    = (last - first + 1) * pitch;
