...
private:
	Shader LoadShaderPassOrExit(const std::string shader_name);

	ShaderPass& GetShaderPass(const ShaderPassId id);

	GLuint CreateTexture(const int width, const int height) const;
	void SetTextureFiltering(const GLuint texture) const;

	void UpdateInputTextureUniforms(const ShaderPass& pass) const;
