...
void ShaderPipeline::CreatePipeline()
{
	...
	for (auto it = shader_passes.begin(); it != shader_passes.end(); ++it) {
		...
		pass.out_texture_size = {0, 0, width, height};
...
void ShaderPipeline::UpdateDeditherPassUniforms()
{
	...
	for (const auto& name : names) {
		...
		// Always on (we set the strength to zero or unload the
		...
		// original and the deithered images while squinting; the
		// dithered areas should have the same perceptual brightness).
		//
		...
		// These options yield the best results for typical DOS game
		// dithering patterns. Enabling CD_MITIG_LINES is important to
		// ensure sharp 1px text and 45-degree diagonals, and raising
