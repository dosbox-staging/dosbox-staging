...
// value types
std::optional<AutoMountSettings> parse_drive_conf(const std::string_view dir_letter,
...
// provided settings
std::string build_auto_mount_fd_images_cmd(const std::string_view dir_letter,
                                           const std::vector<std_fs::path>& image_paths,
                                           const std::optional<AutoMountSettings>& settings)
{
	auto command = CmdMount;
...
std::string build_auto_mount_cmd(const std::string_view dir_letter,
                                 const std_fs::path& drive_path,
                                 const std::optional<AutoMountSettings>& settings)
{
	...
	const auto image_paths = get_files_by_ext(drive_path, image_ext);
	...
	return build_auto_mount_dir_cmd(dir_letter, drive_path, settings);
}
...
void AutoExecModule::AutoMountDetectedDrive(const std::string& dir_letter)
{
	...
	AddLine(Placement::InitialAutogeneratedCommands,
	        build_auto_mount_cmd(dir_letter, drive_path, settings));
