...
void ReadMemCommand::Get(const Request& req, Response& res)
{
	...
	if (req.get_header_value("accept").starts_with(TYPE_JSON)) {
		json j;
		...
		res.set_content(cmd.memory, TYPE_BINARY);
	}
}
...
void WriteMemCommand::Put(const httplib::Request& req, httplib::Response& res)
{
	...
	std::string data;
	if (req.get_header_value("Content-Type") == TYPE_JSON) {
		auto j = json::parse(req.body);
		data   = base64::from_base64(j["data"].get<std::string>());
	} else if (req.get_header_value("Content-Type") == TYPE_BINARY) {
		data = req.body;
	} else {
		throw std::invalid_argument("Content-Type must be either " +
		                            std::string(TYPE_JSON) + " or " +
		                            std::string(TYPE_BINARY));
	}
