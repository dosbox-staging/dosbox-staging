...
constexpr int DOS_BLOCK_SIZE = 16;
...
{
	uint16_t blocks  = (bytes + DOS_BLOCK_SIZE - 1) / DOS_BLOCK_SIZE;
	...
	LOG_DEBUG("API: AllocMemoryCommand(%d): result=%d, %d bytes at %p (DOS allocator)",
	          bytes,
	          ok,
	          blocks * DOS_BLOCK_SIZE,
	          addr);
	DOS_SetMemAllocStrategy(old_strat);

	if (!ok) {
		addr = 0;
	} else if (blocks * DOS_BLOCK_SIZE < bytes) {
...
void FreeMemoryCommand::Execute()
{
	if (addr < XMS_START * MEM_PAGE_SIZE) {
		success = DOS_FreeMemory(addr / DOS_BLOCK_SIZE);
