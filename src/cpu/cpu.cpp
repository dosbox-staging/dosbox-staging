...
Bitu CPU_extflags_toggle = 0; // ID and AC flags may be toggled depending on
...
bool CPU_SwitchTask(Bitu new_tss_selector, TSwitchType tstype, Bitu old_eip)
{
	...
	if (!cpu_tss.SetSelector(new_tss_selector)) {
		...
		("TaskSwitch: set tss selector %X failed", new_tss_selector);
	}
...
void CPU_Interrupt(Bitu num, Bitu type, Bitu oldeip)
{
	...
	if (!cpu.pmode) {
		...
		if ((reg_flags & FLAG_VM) && (type & CPU_INT_SOFTWARE) &&
			...
			//			LOG_MSG("Software int in v86, AH %X
			//IOPL %x",reg_ah,(reg_flags & FLAG_IOPL) >>12);
...
void CPU_IRET(bool use32, Bitu oldeip)
{
	if (!cpu.pmode) { /* RealMode IRET */
		...
		if (n_cs_rpl == cpu.cpl) {
			...
			DestroyConditionFlags();
			LOG(LOG_CPU, LOG_NORMAL)
			("IRET:Same level:%X:%X big %d", n_cs_sel, n_eip, cpu.code.big);
		} else {
			...
			LOG(LOG_CPU, LOG_NORMAL)
			("IRET:Outer level:%X:%X big %d", n_cs_sel, n_eip, cpu.code.big);
		}
		return;
	}
}
	...
	if (!cpu.pmode || (reg_flags & FLAG_VM)) {
		...
		case DESC_CODE_R_C_NA:
			CPU_CHECK_COND(call.DPL() > cpu.cpl,
			               ...
			               selector & 0xfffc)
			LOG(LOG_CPU, LOG_NORMAL)
			("CALL:CODE:C to %X:%X", selector, offset);
		call_code:
			if (!call.saved.seg.p) {
...
void CPU_RET(bool use32, Bitu bytes, [[maybe_unused]] Bitu oldeip)
{
	if (!cpu.pmode || (reg_flags & FLAG_VM)) {
		...
		RET_same_level:
			...
			LOG(LOG_CPU, LOG_NORMAL)
			("RET - Same level to %X:%X RPL %X DPL %X",
			 selector,
			 offset,
			 rpl,
			 desc.DPL());
