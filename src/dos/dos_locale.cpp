...
static DosCountry get_default_country_id()
{
...
static std::string get_locale_period_for_log()
{
	switch (config.locale_period) {
...
void DOS_RefreshCountryInfo(const bool keyboard_layout_changed) // XXX needs a rework
{
	...
	populate_time_date_format(source);
	populate_currency_format(source);

	// Mark locale as generated XXX change to populated
	config.is_locale_generated = true;
}

// XXX
// std::optional<DosCountry> country_id_numeric   = {};
// std::optional<DosCountry> country_id_time_date = {};
// std::optional<DosCountry> country_id_currency  = {};
...
static bool set_country(const DosCountry country, const bool no_fallback = false)
{
	...
	if (!CountryData.contains(country_deduplicated)) {
		...
		const auto default_country = get_default_country_id();
...
bool DOS_SetCountry(const uint16_t country_id)
{
	...
	// XXX log!
	// XXX consider some special handling if 'auto' is set by the user
                         ...
                         const LocalePeriod locale_period)
{
	...
	const auto host_locale = DetectHostLocale();
	DosCountry country_id  = get_default_country_id();

	bool using_default  = false;
	bool using_detected = false;
	if (country_config_str == "auto") {
		if (host_locale.country) {
			country_id = *host_locale.country;
			using_detected = true;
		} else {
			using_default = true;
		}
	} else {
		const auto value = parse_int(country_config_str);
		if (!value || (*value < 0) || (*value > UINT16_MAX)) {
			...
			const auto converted_id = static_cast<DosCountry>(*value);
			if (LocaleData::CountryData.contains(converted_id)) {
				country_id = converted_id;
			} else {
				using_default = true;
			}
		}
	...
	if (using_detected) {
		country_id_numeric   = *host_locale.numeric;
		country_id_time_date = *host_locale.time_date;
		country_id_currency  = *host_locale.currency;
	}

	// XXX apply numeric, currency, etc., rethink the log
	set_country(country_id);

	std::string additional_comment = {};
	if (using_default) {
		additional_comment = " (default)";
	} else if (using_detected) {
		additional_comment = format_str(" (detected from host value '%s')",
		                                host_locale.log_info.country.c_str());
	}

	// XXX warn if couldn't get modern locale
	LOG_INFO("DOS: Loaded %s locale for country %d '%s'%s",
	         get_locale_period_for_log().c_str(),
	         config.country_dos_code,
	         get_country_name_for_log(country_id).c_str(),
...
static void load_locale_period(const LocalePeriod locale_period)
{
	...
	// XXX get_default_country_id depends on locale period!
}
...
DOS_Locale::DOS_Locale(Section* configuration) : Module_base(configuration)
{
	...
	// XXX country can't be changed in runtime, as there is a DOS function to change it
