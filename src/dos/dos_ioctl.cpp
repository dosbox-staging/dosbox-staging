...
bool DOS_IOCTL_AX440D_CH08(uint8_t drive, bool query)
{
	...
	case 0x40: /* Set device parameters */
	{
		...
		if (mem_readw(ptr + 0xd) == 0 && mem_readw(ptr + 0xf) == 0 &&
			...
			bpb.v32.BPB_BytsPerSec = mem_readw(ptr + 7); // bytes
			                                             // per
			                                             // sector
			                                             // (Win3
			                                             // File Mgr.
			                                             // uses it)
			bpb.v32.BPB_SecPerClus = mem_readb(ptr + 9); // sectors
			                                             // per cluster
			bpb.v32.BPB_RsvdSecCnt = mem_readw(ptr + 0xa); // number
			                                               // of
			                                               // reserved
			                                               // sectors
			bpb.v32.BPB_NumFATs = mem_readb(ptr + 0xc); // number of
			                                            // FATs
			bpb.v32.BPB_RootEntCnt = mem_readw(ptr + 0xd); // number
			                                               // of root
			                                               // entries
			                                               // (Fake,
			                                               // the real
			                                               // BPB value
			                                               // is zero)
			bpb.v32.BPB_TotSec16 = mem_readw(ptr + 0xf); // number
			                                             // of small
			                                             // sectors
			                                             // (always
			                                             // zero on
			...
			bpb.v32.BPB_FATSz32 = (uint16_t)mem_readw(
			        ptr + 0x12); // sectors per FAT (FIXME: What does
			...
			bpb.v32.BPB_HiddSec = (uint32_t)mem_readd(
			        ptr + 0x18); // number of hidden sectors
	...
	case 0x60: /* Get device parameters */
		...
		}
		{
			...
			mem_writew(ptr + 2, (drive >= 2) ? 0x01 : 0x00); // attributes:
			                                                 ...
			                                                 // for
			                                                 // nonremovable
			mem_writew(ptr + 4, (drive >= 2) ? 0x3FF : 0x50); // number
			                                                  // of
			                                                  // cylinders
			...
			if (usereal) {
				...
				if (bpb.is_fat32()) {
					/* Windows 98 behavior: Some of the
					 * FAT32 BPB fields are translated into
					 * FAT16 BPB fields even though those
					 * fields are zero in the actual BPB */
					...
					mem_writew(ptr + 0xd, 0x200); // number
					                              // of root
					                              // entries
					                              // (Fake,
					                              // the
					                              ...
					                              // is
					                              // zero)
					mem_writew(ptr + 0xf, 0); // number of
					                          // small
					                          // sectors
					                          // (always
					                          // zero on BPB
					...
					mem_writew(ptr + 0x12,
					           (uint16_t)bpb.v32.BPB_FATSz32); // sectors per FAT (FIXME: What does Win98 do if this value > 0xFFFF?)
					mem_writew(ptr + 0x14,
				...
				mem_writew(ptr + 7, 0x0200); // bytes per sector
				                             // (Win3 File Mgr.
				                             // uses it)
				mem_writew(ptr + 9, (drive >= 2) ? 0x20 : 0x01); // sectors per cluster
				mem_writew(ptr + 0xa, 0x0001); // number of
				                               // reserved sectors
				...
				mem_writed(ptr + 0x18, 0); // number of hidden
				                           // sectors
				mem_writed(ptr + 0x1c,
				           (drive >= 2) ? 0x31F11 : 0x00); // number
				                                           // of
				                                           // big
				                                           // sectors
			}
