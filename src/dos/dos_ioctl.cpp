...
bool DOS_IOCTL_AX440D_CH08(uint8_t drive, bool query)
{
	...
	case 0x40: /* Set device parameters */
	{
		...
		if (fdp->loadedDisk != NULL) {
			// number of cylinders
			fdp->loadedDisk->cylinders = mem_readw(ptr + 4);
		}

		if (mem_readw(ptr + 0xd) == 0 && mem_readw(ptr + 0xf) == 0 &&
		    mem_readw(ptr + 0x12) == 0) { // FAT32 BPB?

			// bytes per sector (Win3 File Mgr. uses it)
			bpb.v32.BPB_BytsPerSec = mem_readw(ptr + 7);

			// sectors per cluster
			bpb.v32.BPB_SecPerClus = mem_readb(ptr + 9);

			// number of reserved sectors
			bpb.v32.BPB_RsvdSecCnt = mem_readw(ptr + 0xa);

			// number of FATs
			bpb.v32.BPB_NumFATs = mem_readb(ptr + 0xc);

			// number of root entries (Fake, the real BPB value is
			// zero)
			bpb.v32.BPB_RootEntCnt = mem_readw(ptr + 0xd);

			// number of small sectors (always zero on BPB and
			// returned by Win98)
			bpb.v32.BPB_TotSec16 = mem_readw(ptr + 0xf);
			bpb.v32.BPB_Media = mem_readb(ptr + 0x11); // media type

			// sectors per FAT (FIXME: What does Win98 do if this
			// value > 0xFFFF?)
			bpb.v32.BPB_FATSz32 = (uint16_t)mem_readw(ptr + 0x12);
			...
			bpb.v32.BPB_HiddSec = (uint32_t)mem_readd(ptr + 0x18);
			bpb.v32.BPB_TotSec32 = (uint32_t)mem_readd(ptr + 0x1c); // number of big sectors
		} else {
			// bytes per sector (Win3 File Mgr. uses it)
			bpb.v.BPB_BytsPerSec = mem_readw(ptr + 7);

			// sectors per cluster
			bpb.v.BPB_SecPerClus = mem_readb(ptr + 9);

			// number of reserved sectors
			bpb.v.BPB_RsvdSecCnt = mem_readw(ptr + 0xa);

			// number of FATs
			bpb.v.BPB_NumFATs = mem_readb(ptr + 0xc);

			// number of root entries
			bpb.v.BPB_RootEntCnt = mem_readw(ptr + 0xd);

			// number of small sectors
			bpb.v.BPB_TotSec16 = mem_readw(ptr + 0xf);
	...
	case 0x60: /* Get device parameters */
		...
		}
		{
			...
			// type: hard disk(5), 1.44 floppy(7)
			mem_writeb(ptr + 1, (drive >= 2) ? 0x05 : 0x07);

			// attributes: bit 0 set for nonremovable
			mem_writew(ptr + 2, (drive >= 2) ? 0x01 : 0x00);

			// number of cylinders
			mem_writew(ptr + 4, (drive >= 2) ? 0x3FF : 0x50);
			...
			if (usereal) {
				...
				if (bpb.is_fat32()) {
					// Windows 98 behavior: Some of the FAT32
					// BPB fields are translated into FAT16
					// BPB fields even though those fields
					// are zero in the actual BPB
					...
					// number of root entries (Fake, the
					// real BPB value is zero)
					mem_writew(ptr + 0xd, 0x200);

					// number of small sectors (always zero
					// on BPB and returned by Win98)
					mem_writew(ptr + 0xf, 0);
					mem_writeb(ptr + 0x11, bpb.v32.BPB_Media); // media type

					// sectors per FAT (FIXME: What does
					// Win98 do if this value > 0xFFFF?)
					mem_writew(ptr + 0x12,
					           (uint16_t)bpb.v32.BPB_FATSz32);
					mem_writew(ptr + 0x14,
					...
					// bytes per sector (Win3 File Mgr. uses
					// it)
					mem_writew(ptr + 7, bpb.v.BPB_BytsPerSec);
					mem_writeb(ptr + 9, bpb.v.BPB_SecPerClus); // sectors per cluster

					// number of reserved sectors
					mem_writew(ptr + 0xa, bpb.v.BPB_RsvdSecCnt);
					mem_writeb(ptr + 0xc, bpb.v.BPB_NumFATs); // number of FATs

					// number of root entries
					mem_writew(ptr + 0xd, bpb.v.BPB_RootEntCnt);

					// number of small sectors
					mem_writew(ptr + 0xf, bpb.v.BPB_TotSec16);
				...
				// number of hidden sectors
				mem_writed(ptr + 0x18, 0);

				// number of big sectors
				mem_writed(ptr + 0x1c, (drive >= 2) ? 0x31F11 : 0x00);
			}
	...
	case 0x42: /* Format and verify logical device track (FORMAT.COM) */
	{
		 ...
		 * FIXME: MS-DOS may not adjust sector value, or maybe it
		 * does... perhaps there is a reason Linux fdisk warns about
		 * sector alignment to sect/track for MS-DOS partitions? */
		{
			...
			/* finally, MS-DOS counts sectors from 0 and BIOS INT
			 * 13h counts from 1 */
			sect++;
		}

		// STUB!
		LOG_INFO("DOS:IOCTL Call 0D:42 Drive %2X pretending to format device track C/H/S=%u/%u/%u ntracks=%u",
		         drive,
		         cyl,
		         head,
		         sect,
	...
	case 0x62: /* Verify logical device track (FORMAT.COM) */
	{
		...
		// STUB!
		LOG_INFO("DOS:IOCTL Call 0D:62 Drive %2X pretending to verify device track C/H/S=%u/%u/%u ntracks=%u",
		         drive,
		         cyl,
		         head,
		         sect,
	...
	case 0x46: /* Set volume serial number */
		...
		}
		{
			...
			FAT_BootSector::bpb_union_t bpb = fdp->GetBPB();
			unsigned long serial_number     = mem_readd(ptr + 2)
			                                        ? mem_readd(ptr + 2)
			                                        : 0x1234;
			if (bpb.is_fat32()) {
	...
	case 0x66: /* Get volume serial number */
		...
		{
			const char* bufin = Drives[drive]->GetLabel();
			char buffer[11];
			memset(buffer, ' ', 11);

			const char* find_ext = strchr(bufin, '.');
			if (find_ext) {
	...
	case 0x41: /* Write logical device track */
	{
		...
		while (nsect > 0) {
			...
			uint8_t status = fdp->loadedDisk->Write_Sector(head,
			                                               cyl,
			                                               sect,
			                                               sectbuf);
			if (status != 0) {
				LOG_INFO("IOCTL 0D:61 write error at C/H/S %u/%u/%u",
				         cyl,
				         head,
				         sect);
	...
	case 0x61: /* Read logical device track */
	{
		...
		while (nsect > 0) {
			uint8_t status = fdp->loadedDisk->Read_Sector(head,
			                                              cyl,
			                                              sect,
			                                              sectbuf);
			if (status != 0) {
				LOG_INFO("IOCTL 0D:61 read error at C/H/S %u/%u/%u",
				         cyl,
				         head,
				         sect);
				DOS_SetError(DOSERR_ACCESS_DENIED); // FIXME
	...
	case 0x67: /* Get access flag (whether allowed by driver) */
		 ...
		 * Real MS-DOS might be more restrictive, especially Windows 95
		 * which requires volume locking before disk access is allowed */
		/* FDISK.EXE needs this IOCTL to determine whether it can read
		 * the partition and therefore whether the "system type" is
		 * "FAT16", "FAT12", "unknown" etc. */
		/* ptr+0 = special function (always zero)
		 * ptr+1 = return whether access allowed */
		mem_writeb(ptr + 1, 0x01);
		break;
	default:
		LOG_WARNING("DOS:IOCTL %s %2X:%2X Drive %2X unhandled (CH=08h)",
		            query ? "Query" : "Call",
		            reg_al,
		            reg_cl,
		            drive);
...
bool DOS_IOCTL_AX440D_CH48(uint8_t drive, bool query)
{
	...
	case 0x40: /* Set device parameters */
	{
		...
		FAT_BootSector::bpb_union_t bpb = fdp->GetBPB();
		if (fdp->loadedDisk != NULL) {
			fdp->loadedDisk->cylinders = mem_readw(ptr + 4); // number
			                                                 // of
			                                                 // cylinders
		}
		...
		if (mem_readw(ptr + 0xd) == 0 && mem_readw(ptr + 0xf) == 0 &&
		    mem_readw(ptr + 0x12) == 0) {                  // FAT32 BPB?
			bpb.v.BPB_BytsPerSec = mem_readw(ptr + 7); // bytes per
			                                           // sector
			                                           // (Win3 File
			                                           // Mgr. uses
			                                           // it)
			bpb.v.BPB_SecPerClus = mem_readb(ptr + 9); // sectors per
			                                           // cluster
			bpb.v.BPB_RsvdSecCnt = mem_readw(ptr + 0xa); // number
			                                             // of
			                                             // reserved
			                                             // sectors
			bpb.v.BPB_NumFATs = mem_readb(ptr + 0xc); // number of FATs
			bpb.v.BPB_RootEntCnt = mem_readw(ptr + 0xd); // number
			                                             // of root
			                                             // entries
			bpb.v.BPB_TotSec16 = mem_readw(ptr + 0xf); // number of
			                                           // small sectors
			bpb.v.BPB_Media = mem_readb(ptr + 0x11); // media type
			bpb.v.BPB_FATSz16 = (uint16_t)mem_readw(ptr + 0x12); // sectors per FAT
			bpb.v.BPB_SecPerTrk = (uint16_t)mem_readw(ptr + 0x14); // sectors per track
	...
	case 0x60: /* Get device parameters */
		...
		{
			// mem_writeb(ptr+0,0); // special functions (call value)
			mem_writeb(ptr + 1, (drive >= 2) ? 0x05 : 0x07); // type:
			                                                 // hard
			                                                 // disk(5),
			                                                 // 1.44
			                                                 // floppy(7)
			mem_writew(ptr + 2, (drive >= 2) ? 0x01 : 0x00); // attributes:
			                                                 // bit
			                                                 // 0 set
			                                                 // for
			                                                 // nonremovable
			mem_writew(ptr + 4, (drive >= 2) ? 0x3FF : 0x50); // num
			                                                  // of
			                                                  // cylinders
			mem_writeb(ptr + 6, 0x00); // media type (00=other type)
			// bios parameter block following
			...
			if (usereal) {
				if (fdp->loadedDisk != NULL) {
					mem_writew(ptr + 4,
					           fdp->loadedDisk->cylinders); // num of cylinders
				}

				if (bpb.is_fat32()) {
					mem_writew(ptr + 7,
					           bpb.v.BPB_BytsPerSec); // bytes
					                                  // per
					                                  // sector
					                                  // (Win3
					                                  // File
					...
					mem_writeb(ptr + 9, bpb.v.BPB_SecPerClus); // sectors per cluster
					mem_writew(ptr + 0xa,
					           bpb.v.BPB_RsvdSecCnt); // number
					                                  // of
					                                  // reserved
					                                  // sectors
					mem_writeb(ptr + 0xc, bpb.v.BPB_NumFATs); // number of FATs
					mem_writew(ptr + 0xd,
					           bpb.v.BPB_RootEntCnt); // number
					                                  // of
					                                  // root
					                                  // entries
					mem_writew(ptr + 0xf,
					           bpb.v.BPB_TotSec16); // number
					                                // of
					                                // small
					                                // sectors
					mem_writeb(ptr + 0x11, bpb.v.BPB_Media); // media type
					...
					mem_writew(ptr + 0x24,
					           (uint16_t)bpb.v32.BPB_ExtFlags);
					mem_writew(ptr + 0x26,
					           (uint16_t)bpb.v32.BPB_FSVer);
					mem_writed(ptr + 0x28,
					           (uint32_t)bpb.v32.BPB_RootClus);
					mem_writew(ptr + 0x2C,
					           (uint16_t)bpb.v32.BPB_FSInfo);
					mem_writew(ptr + 0x2E,
					           (uint16_t)bpb.v32.BPB_BkBootSec);
				} else {
					DOS_SetError(DOSERR_ACCESS_DENIED);
					return false;
				}
	...
	case 0x66:
	case 0x6A:
	case 0x6B: return DOS_IOCTL_AX440D_CH08(drive, query);
	default:
		LOG(LOG_IOCTL, LOG_ERROR)
		("DOS:IOCTL Call %02X:%2X Drive %2X unhandled (CH=48h)",
		 reg_al,
		 reg_cl,
		 drive);
		DOS_SetError(DOSERR_FUNCTION_NUMBER_INVALID);
...
//	LOG(LOG_IOCTL,LOG_WARN)("%X %X %X %X",reg_ax,reg_bx,reg_cx,reg_dx);
	...
	case 0x11:              /* query generic ioctl capability */
	{
		...
		if (Drives[drive]->isRemovable()) {
			LOG_INFO("Attempt IOCTL AX=%04x CX=%04x on removable drive",
			         reg_ax,
			         reg_cx);
			DOS_SetError(DOSERR_FUNCTION_NUMBER_INVALID);
			return false;
		...
		if (reg_ch == 0x08) {
			...
			return DOS_IOCTL_AX440D_CH48(drive,
			                             reg_al == 0x11); // Same
			                                              // functions
			                                              // as
			                                              // CH=08h
