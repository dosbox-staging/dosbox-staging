        ...
        if (imgsizedetect) {
			...
			if (!DetectMFMsectorPartition(buf, fcsize, sizes)) {
				...
				if (ptype == 0x0C/*FAT32+LBA*/ || ptype == 0x0E/*FAT16+LBA*/) {
					assume_lba = true;
					LOG_WARNING("IMGMOUNT: Failed to autodetect geometry, assuming LBA approximation based on first partition type (FAT with LBA)");
				}
				if (!assume_lba && (ptype == 0x01 || ptype == 0x04 || ptype == 0x06 || ptype == 0x0B || ptype == 0x0C || ptype == 0x0E)) {
					...
					if (i == 0x20) {
						assume_lba = true;
						LOG_WARNING("IMGMOUNT: Failed to autodetect geometry, assuming LBA approximation based on first partition type (FAT-related) and lack of executable code in the MBR");
					}
				}
				if (!assume_lba && fcsize >= ((4ull*1024ull*1024ull*1024ull)/512ull)) {
					assume_lba = true;
					LOG_WARNING("IMGMOUNT: Failed to autodetect geometry, assuming LBA approximation based on size");
				}
				if (assume_lba) {
					sizes[0] = 512;
					sizes[1] = 63;
