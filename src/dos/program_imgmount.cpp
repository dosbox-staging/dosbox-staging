		...
		if (imgsizedetect) {
			...
			if (!DetectMFMsectorPartition(buf, fcsize, sizes)) {
				...
				if (ptype == 0x0C /*FAT32+LBA*/ ||
					...
					assume_lba = true;
					DEBUG_LOG_MSG("IMGMOUNT: Failed to autodetect geometry, assuming LBA approximation based on first partition type (FAT with LBA)");
				}
				if (!assume_lba &&
					...
					if (i == 0x20) {
						assume_lba = true;
						DEBUG_LOG_MSG("IMGMOUNT: Failed to autodetect geometry, assuming LBA approximation based on first partition type (FAT-related) and lack of executable code in the MBR");
					}
				}
				if (!assume_lba &&
				    fcsize >= ((4ull * 1024ull * 1024ull * 1024ull) /
				               512ull)) {
					assume_lba = true;
					DEBUG_LOG_MSG("IMGMOUNT: Failed to autodetect geometry, assuming LBA approximation based on size");
				}
				if (assume_lba) {
					sizes[0]     = 512;
					sizes[1]     = 63;
