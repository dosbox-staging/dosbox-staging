          ...
          curFatSect(0)
{
	...
	if (bootbuffer.sectorsperfat == 0) {
		...
		created_successfully = false;
		LOG_MSG("Loaded image has zero sectors per FAT! FAT32 and non-FAT filesystems are not supported.");
		return;
	}
	if (bootbuffer.bytespersector != BytePerSector) {
		created_successfully = false;
		LOG_MSG("Bytes Per Sector mismatch: expected %u, got %u",
		        BytePerSector,
	...
	if (bootbuffer.sectorspercluster == 0) {
		created_successfully = false;
		LOG_MSG("Loaded image has zero sectors per cluster!");
		return;
	}
	if (bootbuffer.rootdirentries == 0) {
		created_successfully = false;
		LOG_MSG("Loaded image has zero root directory entries!");
		return;
	}
	if (bootbuffer.fatcopies == 0) {
		created_successfully = false;
		LOG_MSG("Loaded image has zero FAT copies!");
		return;
	}
	/* Check geometry values */
	if (bootbuffer.headcount == 0) {
		created_successfully = false;
		LOG_MSG("Loaded image has zero heads per cylinder!");
		return;
	}
	if (bootbuffer.headcount > headscyl) {
		created_successfully = false;
		LOG_MSG("Loaded image has more heads per cylinder (%u) than the disk geometry allows (%u)!",
	...
	if (bootbuffer.sectorspertrack == 0) {
		created_successfully = false;
		LOG_MSG("Loaded image has zero sectors per track!");
		return;
	}
	if (bootbuffer.sectorspertrack > cylsector) {
		created_successfully = false;
		LOG_MSG("Loaded image has more sectors per track (%u) than the disk geometry allows (%u)!",
