...
void MOUSE::Run()
{
	...
	if (cmd->FindExistRemoveAll("/z") | cmd->FindExistRemoveAll("/b") |
	    cmd->FindExistRemoveAll("/i1") | cmd->FindExistRemoveAll("/i2") |
	    cmd->FindExistRemoveAll("/c1") | cmd->FindExistRemoveAll("/c2") |
	    cmd->FindExistRemoveAll("/c3") | cmd->FindExistRemoveAll("/c4") |
	    cmd->FindExistRemoveAll("/f")) {
...
void MOUSE::AddMessages()
{
	MSG_Add("PROGRAM_MOUSE_HELP_LONG",
	        ...
	        "\n"
	        "Notes:\n"
	        "  The driver uses the host mouse directly, it works separately from the PS/2 or\n"
	        "  serial (COM port) mice.\n");
