...
void MOUNT::Run(void)
{
	...
	bool iscdrom = (type == "cdrom"); // Used for mscdex bug cdrom label
	                                  // name emulation
	...
	if (type == "floppy" || type == "dir" || type == "cdrom" || type == "overlay") {
		...
		if (cmd->FindString("-freesize", mb_size, true)) {
			...
			if (type == "floppy") {
				...
				uint32_t free_size_cyl  = (uint32_t)freesize *
				                         1024 * 1024 / (512 * 32);
		...
		if (real_path.empty()) {
			...
			if (home_resolve == real_path) {
				LOG_MSG("MOUNT: Path '%s' found", temp_line.c_str());
			} else {
		...
		if (stat(temp_line.c_str(), &test)) {
			WriteOut(MSG_Get("PROGRAM_MOUNT_ERROR_1"),
			         temp_line.c_str());
			return;
		}
		/* Not a switch so a normal directory/file */
		if (!S_ISDIR(test.st_mode)) {
			WriteOut(MSG_Get("PROGRAM_MOUNT_ERROR_2"),
			         temp_line.c_str());
			return;
		}
		...
		if (type == "cdrom") {
			// Following options were relevant only for physical
			// CD-ROM support:
			for (auto opt :
			     {"-noioctl", "-ioctl", "-ioctl_dx", "-ioctl_mci", "-ioctl_dio"}) {
