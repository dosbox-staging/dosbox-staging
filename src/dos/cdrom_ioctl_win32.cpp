...
bool CDROM_Interface_Ioctl::GetAudioTracks(uint8_t& stTrack, uint8_t& endTrack, TMSF& leadOut) {
	CDROM_TOC toc;
	DWORD	byteCount;
...
bool CDROM_Interface_Ioctl::GetAudioTrackInfo(uint8_t track, TMSF& start, unsigned char& attr) {
	CDROM_TOC toc;
	DWORD	byteCount;
	BOOL	bStat = DeviceIoControl(hIOCTL,IOCTL_CDROM_READ_TOC, NULL, 0, 
									&toc, sizeof(toc), &byteCount,NULL);
...
bool CDROM_Interface_Ioctl::ReadSectors(PhysPt buffer, bool raw, const uint32_t sector, const uint16_t num) {
	BOOL  bStat;
	DWORD byteCount = 0;
