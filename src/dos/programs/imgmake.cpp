...
std::array<uint8_t, 3> lba_to_chs(int64_t lba, int max_cylinders,
                                  int max_heads, int max_sectors)
{
...
bool execute(Program* program, CommandSettings& command_settings)
{
    ...
    boot_sector->media_descriptor = static_cast<uint8_t>(disk_geometry.media_descriptor);
    ...
    auto tmp_data_sectors = volume_sectors - reserved_sectors -
                                root_dir_sectors;
    ...
    if (fat_bits == 32) {
        ...
        int string_addr = BootCode::PhysicalAddress +
                               BootCode::OffsetFAT32 +
                               BootCode::StringOffsetInCode;
        host_writew(reinterpret_cast<uint8_t*>(boot_sector->ext.fat32.boot_code + BootCode::PatchOffsetInCode),
