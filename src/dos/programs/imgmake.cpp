...
bool execute(Program* program, CommandSettings& command_settings)
{
	...
	constexpr auto SectorSizeBytes = 512;
	constexpr int64_t SectorsPerMB = (1024 * 1024) / SectorSizeBytes;
	constexpr int64_t SectorsPerGB = (1024 * 1024 * 1024) / SectorSizeBytes;
	...
	if (auto it = GeometryPresets.find(command_settings.type);
		...
		if (!command_settings.use_chs && command_settings.size_bytes == 0) {
	...
	auto volume_sectors = static_cast<int64_t>(
	        (total_size / SectorSizeBytes) - boot_sector_position);
	...
	if (!disk_geometry.is_floppy) {
		...
		// Partition 1 Entry
		auto* partition = static_cast<uint8_t*>(buffer.data() +
		                                        PartitionEntry1Offset);
		...
		// End CHS
		auto end_chs = lba_to_chs((total_size / SectorSizeBytes) - 1,
		                          disk_geometry.cylinders,
		                          disk_geometry.heads,
		                          disk_geometry.sectors);
	...
	write_padded_string(reinterpret_cast<uint8_t*>(boot_sector->oem_name),
	                    "DOSBOX-S",
	                    8);
	write_le(&boot_sector->bytes_per_sector, SectorSizeBytes);

	// Calculate Sectors Per Cluster (SPC) Safely
	auto sectors_per_cluster = (command_settings.sectors_per_cluster > 0)
	...
	if (fat_bits != 32) {
		...
		root_dir_sectors_temp = static_cast<uint32_t>(
		        ((static_cast<uint32_t>(root) * 32) + (SectorSizeBytes - 1)) /
		        SectorSizeBytes);
	}
	...
	auto fat_limit = (fat_bits == 12) ? FatMarkers::MaxClustersFat12
	               : (fat_bits == 16) ? FatMarkers::MaxClustersFat16
	                                  : MaxFatLimit;
	...
	int root_dir_sectors = ((root_ent * 32) + (SectorSizeBytes - 1)) /
	                       SectorSizeBytes;
	...
	fat_size_sectors = static_cast<uint32_t>(
	        (fat_size_bytes + SectorSizeBytes - 1) / SectorSizeBytes);
	...
	write_le(&boot_sector->total_sectors_32,
	         (fat_bits == 32 || volume_sectors > UINT16_MAX) ? volume_sectors
	                                                         : 0);
	...
	if (fat_bits == 32) {
		...
		std::array<uint8_t, SectorSizeBytes> fs_info_buffer = {};
		...
		fwrite(fs_info_buffer.data(), 1, SectorSizeBytes, fs.get());
	...
	// FAT Initialization
	std::array<uint8_t, SectorSizeBytes> fat_sector_buffer = {};
	std::array<uint8_t, SectorSizeBytes> empty_sector      = {};
	...
	if (!command_settings.label.empty()) {
		std::array<uint8_t, SectorSizeBytes> root_buffer = {};
