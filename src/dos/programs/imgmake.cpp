...
std::array<uint8_t, 3> lba_to_chs(uint64_t lba, uint32_t max_cylinders,
...
bool Execute(Program* program, CommandSettings& command_settings)
{
	...
	constexpr auto SectorsPerMB = (1024 * 1024) / SectorSize;
	constexpr auto SectorsPerGB = (1024 * 1024 * 1024) / SectorSize;
	...
	if (auto it = GeometryPresets.find(command_settings.type);
		...
		if (command_settings.use_chs) {
			...
			if (total_size > 4ULL * BytesPerGigabyte) {
			...
			disk_geometry.cylinders = std::min(disk_geometry.cylinders,
			                                   static_cast<uint32_t>(1023));

			// Recalculate total_size to align exactly with the
			// geometry. If we don't do this, the file will contain
			// "slack" bytes at the end that don't fit into a
			// cylinder, causing imgmount to fail geometry checks.
			total_size = static_cast<uint64_t>(disk_geometry.cylinders) *
			             disk_geometry.heads *
			             disk_geometry.sectors * SectorSize;
		} else {
	...
	uint64_t volume_sectors = (total_size / SectorSize) -
	                          static_cast<uint64_t>(boot_sector_position);
	int fat_bits = command_settings.fat_type;

	// Auto-detect FAT bits for the configured DOS Version
	if (fat_bits == -1) {
		constexpr uint64_t SectorsPerMB = (1024 * 1024) / SectorSize;
		constexpr uint64_t SectorsPerGB = (1024 * 1024 * 1024) / SectorSize;
		// Thresholds
		// FAT12 Limit: ~16 MB
		...
		constexpr auto FAT16EfficiencyLimit = 512 * SectorsPerMB;

		// FAT16 Hard Limit: 2 GB
		// The mathematical limit of unsigned 16-bit math + 32KB clusters.
		constexpr auto FAT16HardLimit = 2ULL * SectorsPerGB;
	...
	if (!disk_geometry.is_floppy) {
		...
		auto start_chs = lba_to_chs(static_cast<uint64_t>(boot_sector_position),
	...
	boot_sector->media_descriptor = disk_geometry.media_descriptor;
	...
	uint64_t tmp_data_sectors = volume_sectors - reserved_sectors -
	                            root_dir_sectors;
	uint64_t tmp_clusters = tmp_data_sectors / sectors_per_cluster;

	// The FAT must track every data cluster plus two reserved entries
	// at the start (Index 0: Media Descriptor, Index 1: EOC/Flags).
	constexpr uint64_t ReservedFATEntries = 2;
	uint64_t total_entries = tmp_clusters + ReservedFATEntries;

	uint64_t fat_size_bytes = 0;
	...
	for (int i = 0; i < command_settings.fat_copies; ++i) {
		...
		for (uint32_t k = 1; k < fat_size_sectors; ++k) {
