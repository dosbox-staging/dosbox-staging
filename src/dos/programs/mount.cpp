...
void MOUNT::Run(void) {
	...
	if (type=="floppy" || type=="dir" || type=="cdrom" || type =="overlay") {
		...
		if (type=="floppy") {
			...
			// This is preferable to using type because floppies can be 
			// auto-mounted as type"dir"
			std::string command_arg;
			cmd->FindCommand(1,command_arg);
			const unsigned char first_char = static_cast<unsigned char>(command_arg[0]);
