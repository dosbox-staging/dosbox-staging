...
bool MOUNT::MountImage(MountParameters& params)
{
	...
	if (params.fstype == FileSystemType::Fat16) {
		return MountImageFat(params);

	} else if (params.fstype == FileSystemType::Iso) {
		return MountImageIso(params);

	} else if (params.fstype == FileSystemType::None) {
...
static std::optional<FileSystemType> parse_file_system_type(const std::string s)
{
	if (s == "fat") {
		return FileSystemType::Fat16;

	} else if (s == "iso") {
		return FileSystemType::Iso;
