...
void MEM::Run()
{
	...
	// clutters the output with imformation which is often not needed.
...
std::string MEM::DisplaySummary(MoreOutputStrings& output) const
{
	...
	if (hma.is_available) {
		display_row("PROGRAM_MEM_TYPE_HMA", hma.free_bytes, HmaSize);
	}
	if (xms.is_available) {
...
{
	return format_number((value + KiloByte / 2) / KiloByte) + "K";
}
...
void MEM::ReadBasicMemoryInfo(MemoryInfo& memory) const
{
	...
	auto detect_reserved_mcb = [](McbChainInfoEntry& first) {
		...
		if ((segment_after_start * RealSegmentSize) % KiloByte == 0) {
			first.reserved = true;
		}
	};
...
MEM::XmsInfo MEM::GetXmsInfo() const
{
	...
	if (xms.version_major >= 3 && is_cpu_386_or_better()) {
		...
		CALLBACK_RunRealFar(xms.api_segment, xms.api_offset);
		if (reg_bl == 0) {
			xms.free_bytes         = reg_edx * KiloByte;
			xms.largest_free_block = reg_eax * KiloByte;
		}
	} else {
		reg_ah = 0x08;
		CALLBACK_RunRealFar(xms.api_segment, xms.api_offset);
		if (reg_bl == 0) {
			xms.free_bytes         = reg_dx * KiloByte;
			xms.largest_free_block = reg_ax * KiloByte;
		}
	}
	...
	for (const auto& entry : GetBiosMemoryMap()) {
		...
		constexpr uint64_t XmsBase = KiloByte * KiloByte;
	...
	if (!xms.total_bytes.has_value()) {
		...
		if (!(cpu_regs.flags & FLAG_CF)) {
			xms.total_bytes = (reg_ax + reg_bx) * KiloByte;
		}
	}
	...
	if (xms.total_bytes.has_value() && xms.hma.is_available) {
		if (*xms.total_bytes <= HmaSize) {
			xms.total_bytes = 0;
		} else {
			xms.total_bytes = *xms.total_bytes - HmaSize;
		}
	}
...
void MEM::AddMessages()
{
	MSG_Add("PROGRAM_MEM_HELP_LONG",
	        ...
	        "                   structures.\n"
	        "  /f or /free      display the free memory segments.\n"
	        "  /m or /module    display memory usage of the [color=light-cyan]MODULE[reset] specified.\n"
	        "  /x or /xms       display Extended (XMS) memory usage.\n"
	        "  /e or /ems       display Expanded (EMS) memory usage.\n"
	        "\n"
	        "Notes:\n"
	...
	MSG_Add("PROGRAM_MEM_LABEL_LARGEST_UMB",
	        "Largest free upper (UMB) memory block");
	...
	MSG_Add("PROGRAM_MEM_FREE_TITLE_UPPER",
	        "Free segments in Upper (UMB) memory:");
	...
	MSG_Add("PROGRAM_MEM_MODULE_TITLE",
	        "Module '%s' (PSP segment %04Xh) is using the following memory:");
	...
	MSG_Add("PROGRAM_MEM_EMS_TITLE",
	        "Expanded (EMS) memory detailed information:");
	...
	MSG_Add("PROGRAM_MEM_ERROR_NO_XMS", "No Extended (XMS) memory found.\n");
	MSG_Add("PROGRAM_MEM_ERROR_NO_EMS", "No Expanded (EMS) memory found.\n");
}
