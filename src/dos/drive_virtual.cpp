...
char* VFILE_Generate_8x3(const char* name, const unsigned int onpos)
{
	...
	while (1) {
		...
		auto found = find_file_by_predicate(first_file, [onpos](vfile_block_t cur_file) {
			return onpos == cur_file->onpos &&
			       iequals(sfn, cur_file->name);
		});
                    ...
                    const char *dir)
{
	...
	auto found = find_file_by_predicate(first_file, [onpos, name](vfile_block_t cur_file) {
		return onpos == cur_file->onpos && iequals(name, cur_file->name);
	});
	if (found) {
		return;
	}
...
bool Virtual_Drive::TestDir(char * dir) {
	...
	if (find_file_by_predicate(first_file, [dir](vfile_block_t cur_file) {
		    return cur_file->isdir && iequals(cur_file->name, dir);
	    })) {
		return true;
	}
	return false;
...
bool Virtual_Drive::FileStat(const char* name, FileStat_Block * const stat_block){
	...
	auto found = find_file_by_name(name);
	if (found) {
		stat_block->attr = (int)(found->isdir ? DOS_ATTR_DIRECTORY
		                                      : DOS_ATTR_ARCHIVE);
...
bool Virtual_Drive::equal_name(vfile_block_t cur_file, const char* search_name)
{
	...
	return strcasecmp(search_name,
	                  (std::string(onpos ? vfilenames[onpos].shortname +
	                                               std::string(1, '\\')
	                                     : "") +
	                   cur_file->name)
	                          .c_str()) == 0;
...
{
	return find_file_by_predicate(first_file,
	                              [this, search_name](vfile_block_t cur_file) {
		                              return equal_name(cur_file, search_name);
	                              });
}
