...
constexpr uint32_t MaxFileSize = 1024 * 1024;

constexpr uint8_t IdMsDosWinNt = 0xff;
constexpr uint8_t IdDrDos      = 0x7f;

static const std::string SignatureMsDos = "FONT   ";
static const std::string SignatureWinNt = "FONT.NT";
static const std::string SignatureDrDos = "DRFONT ";

static const std::string SignatureUpx = "UPX!";

static const std::string DeviceEga = "EGA";
static const std::string DeviceLcd = "LCD";
static const std::set<std::string> KnownPrinterDevices =
...
protected:
	...
	std::vector<uint8_t> ReadBlob(const uint32_t length);
	...
	virtual void WarnDataOutOfBounds(const uint32_t offset_start,
	                                 const uint32_t offset_end,
	                                 const std::string& struct_name = {}) = 0;
	virtual void WarnStructSizeInvalid(const uint32_t offset,
...
private:
	...
	uint16_t InternalReadShort();
	uint32_t InternalReadLong();
	std::string InternalReadString(const uint32_t length);
...
uint8_t CpiReader::InternalReadByte()
{
...
uint16_t CpiReader::InternalReadShort()
{
	const auto idx = offset;
...
uint32_t CpiReader::InternalReadLong()
{
	const auto idx = offset;
...
std::string CpiReader::InternalReadString(const uint32_t length)
{
	const auto idx = offset;
...
std::vector<uint8_t> CpiReader::ReadBlob(const uint32_t length)
{
	const auto idx = offset;

	offset += length;
	if (IsOffsetBeyondContent()) {
		WarnDataOutOfBounds(idx, offset);
		return {};
	}

	return std::vector<uint8_t>(content.begin() + idx,
	                            content.begin() + idx + length);
}
...
std::optional<FontFileHeader> CpiReader::ReadFontFileHeader()
{
	...
	result.id_byte   = InternalReadByte();
	result.id_string = InternalReadString(7);
	SkipBytes(8);
	result.num_pointers = InternalReadShort();
	result.pointer_type = InternalReadByte();
	result.fih_offset   = InternalReadLong();

	if (IsOffsetBeyondContent()) {
		WarnDataOutOfBounds(result.struct_offset, offset, result.StructName);
		return {};
	}
	return result;
}
...
std::optional<FontFileExtendedHeader> CpiReader::ReadFontFileExtendedHeader()
{
	...
	result.fonts_per_codepage = InternalReadByte();

	for (size_t idx = 0; idx < result.fonts_per_codepage; ++idx) {
		result.cell_size.push_back(InternalReadByte());
	}

	for (size_t idx = 0; idx < result.fonts_per_codepage; ++idx) {
		result.dfd_offset.push_back(InternalReadLong());
	}
...
std::optional<FontInfoHeader> CpiReader::ReadFontInfoHeader()
{
	...
	result.num_code_pages = InternalReadShort();
...
std::optional<CodePageEntryHeader> CpiReader::ReadCodePageEntryHeader()
{
	...
	result.cpeh_size        = InternalReadShort();
	result.next_cpeh_offset = InternalReadLong();
	result.device_type      = InternalReadShort();
	result.device_name      = InternalReadString(8);
	result.code_page        = InternalReadShort();
	SkipBytes(6);
	...
	if (size == 26) {
		result.cpih_offset = InternalReadShort();
	} else {
		if (size != 28 && !DummySizeValues.contains(size)) {
			WarnStructSizeInvalid(result.struct_offset,
			                      result.StructName,
			                      size);
		}
		result.cpih_offset = InternalReadLong();
	}
...
std::optional<CodePageInfoHeader> CpiReader::ReadCodePageInfoHeader()
{
	...
	result.version   = InternalReadShort();
	result.num_fonts = InternalReadShort();
	result.size      = InternalReadShort();
...
std::optional<ScreenFontHeader> CpiReader::ReadScreenFontHeader()
{
	...
	result.height    = InternalReadByte();
	result.width     = InternalReadByte();
	result.y_aspect  = InternalReadByte();
	result.x_aspect  = InternalReadByte();
	result.num_chars = InternalReadShort();
...
std::optional<CharacterIndexTable> CpiReader::ReadCharacterIndexTable(const uint32_t num_chars)
{
	...
	for (uint32_t idx = 0; idx < num_chars; ++idx) {
		result.font_index.push_back(InternalReadShort());
	}
...
struct ParserResult {
	...
	// If the reson of parsing failure was an unsupported UPX compression
...
private:
	...
	void StoreFontType(const CodePageEntryHeader& header, ParserResult& result);
	...
	void MaybePrintWarningHeader();
	void PrintWarning(const std::string& message);
	void PrintWarning(const uint32_t struct_offset, const std::string& message);
	void PrintWarning(const uint32_t struct_offset, const std::string& struct_name,
	                  const std::string& message);
	void PrintWarning(const uint32_t offset_start,
	                  const uint32_t offset_end, const std::string& message);

	// Warnings, generic

	void WarnDataOutOfBounds(const uint32_t start_offset,
	                         const uint32_t end_offset,
	                         const std::string& struct_name) override;
	void WarnStructSizeInvalid(const uint32_t offset,
	                           const std::string& struct_name,
	                           const uint32_t size) override;
	void WarnOffsetPointingBackwards(const uint32_t struct_offset,
	                                 const std::string& struct_name);
	void WarnStructNotDirectlyAfterPrevious(const uint32_t struct_offset,
	                                        const std::string& struct_name,
	                                        const std::string& predecessor_name);

	// Warnings, device independend

	void WarnInvalidFileHeader(const uint32_t struct_offset,
	                           const std::string& struct_name);
	void WarnNoCodePages(const uint32_t struct_offset,
	                     const std::string& struct_name);
	void WarnInvalidCodePage(const uint32_t struct_offset,
	                         const std::string& struct_name,
	                         const uint16_t code_page);
	void WarnInvalidDeviceName(const uint32_t struct_offset,
	                           const std::string& struct_name);
	void WarnUnknownDeviceType(const uint32_t struct_offset,
	                           const std::string& struct_name);
	void WarnNonEgaInWinNtFile(const uint32_t struct_offset,
	                           const std::string& struct_name);
	void WarnDuplicatedFont(const uint32_t struct_offset,
	                        const std::string& struct_name,
	                        const std::string& device_name,
	                        const uint32_t code_page);
	void WarnMultipleDevicesInSameFile();
	void WarnScreenPrinterInSameFile();

	// Warnings, screen font related

	void WarnScreenFontDataSize(const uint32_t font_offset,
	                            const uint32_t expected, const uint32_t real);
	void WarnFontSizeDuplicateOffsets(const uint32_t struct_offset,
	                                  const std::string& struct_name,
	                                  const uint8_t height);
	void WarnUnknownScreenFontFormat(const uint32_t struct_offset,
	                                 const std::string& struct_name,
	                                 const uint16_t version);
	void WarnScreenFontWithAspect(const uint32_t struct_offset,
	                              const std::string& struct_name);
	void WarnScreenFontWidthInvalid(const uint32_t offset,
	                                const std::string& struct_name,
	                                const uint8_t width);
	void WarnScreenFontDuplicateSize(const uint32_t struct_offset,
	                                 const std::string& struct_name,
	                                 const uint8_t height);
	void WarnScreenFontWithoutSizes(const uint32_t struct_offset,
	                                const std::string& struct_name);
	void WarnScreenFontWithoutUsefulSizes(const uint32_t font_offset);
	void WarnScreenFontSizeWithoutCharacters(const uint32_t struct_offset,
	                                         const std::string& struct_name);

	void WarnUnknownScreenFontDevice(const uint32_t struct_offset,
	                                 const std::string& struct_name,
	                                 const std::string& device_name);

	// Warnings, printer font related

	void WarnPrinterInNonMsDosFile(const uint32_t struct_offset,
	                               const std::string& struct_name);
};

// ***************************************************************************
// CPI file parser - warnings support, generic part
// ***************************************************************************

void CpiParser::MaybePrintWarningHeader()
{
...
void CpiParser::PrintWarning(const std::string& message)
{
	MaybePrintWarningHeader();
	LOG_WARNING("LOCALE: - %s", message.c_str());
}

void CpiParser::PrintWarning(const uint32_t struct_offset, const std::string& message)
{
	MaybePrintWarningHeader();
	LOG_WARNING("LOCALE: - [0x%08x]: %s", struct_offset, message.c_str());
}

void CpiParser::PrintWarning(const uint32_t struct_offset,
                             const std::string& struct_name,
                             const std::string& message)
{
	MaybePrintWarningHeader();
...
void CpiParser::PrintWarning(const uint32_t start_offset,
                             const uint32_t end_offset, const std::string& message)
{
	MaybePrintWarningHeader();
...
// ***************************************************************************
// CPI file parser - warnings, generic
// ***************************************************************************

void CpiParser::WarnDataOutOfBounds(const uint32_t start_offset,
                                    const uint32_t end_offset,
                                    const std::string& struct_name)
{
	std::string message = {};
	...
	PrintWarning(start_offset, end_offset, message);
}

void CpiParser::WarnStructSizeInvalid(const uint32_t struct_offset,
                                      const std::string& struct_name,
                                      const uint32_t size)
{
	const auto message = format_str("invalid size %d", size);
	PrintWarning(struct_offset, struct_name, message);
}

void CpiParser::WarnOffsetPointingBackwards(const uint32_t struct_offset,
                                            const std::string& struct_name)
{
	PrintWarning(struct_offset, struct_name, "offset is pointing backwards");
}

void CpiParser::WarnStructNotDirectlyAfterPrevious(const uint32_t struct_offset,
                                                   const std::string& struct_name,
                                                   const std::string& predecessor_name)
{
	const auto message = format_str("did not come directly after '%s'",
	                                predecessor_name.c_str());
	PrintWarning(struct_offset, struct_name, message);
}

// ***************************************************************************
// CPI file parser - warnings, device independend
// ***************************************************************************

void CpiParser::WarnInvalidFileHeader(const uint32_t struct_offset,
                                      const std::string& struct_name)
{
	PrintWarning(struct_offset, struct_name, "invalid file pointer");
}

void CpiParser::WarnNoCodePages(const uint32_t struct_offset,
                                const std::string& struct_name)
{
	PrintWarning(struct_offset, struct_name, "no code pages in the file");
}

void CpiParser::WarnInvalidCodePage(const uint32_t struct_offset,
                                    const std::string& struct_name,
                                    const uint16_t code_page)
{
	const auto message = format_str("invalid code page %d", code_page);
	PrintWarning(struct_offset, struct_name, message);
}

void CpiParser::WarnInvalidDeviceName(const uint32_t struct_offset,
                                      const std::string& struct_name)
{
	PrintWarning(struct_offset, struct_name, "invalid device name");
}

void CpiParser::WarnUnknownDeviceType(const uint32_t struct_offset,
                                      const std::string& struct_name)
{
	PrintWarning(struct_offset, struct_name, "unknown device type");
}

void CpiParser::WarnNonEgaInWinNtFile(const uint32_t struct_offset,
                                      const std::string& struct_name)
{
	PrintWarning(struct_offset,
	             struct_name,
	             "non-EGA screen font, but the file format is 'Windows NT'");
}

void CpiParser::WarnDuplicatedFont(const uint32_t struct_offset,
                                   const std::string& struct_name,
                                   const std::string& device_name,
                                   const uint32_t code_page)
{
	const auto message = format_str("duplicated device '%s' font for code page %d",
	                                device_name.c_str(),
	                                code_page);
	PrintWarning(struct_offset, struct_name, message);
}

void CpiParser::WarnMultipleDevicesInSameFile()
{
	if (already_warned.multiple_devices_in_same_file) {
		return;
	}

	PrintWarning("found fonts for multiple devices in the same file");
	already_warned.multiple_devices_in_same_file = true;
}

void CpiParser::WarnScreenPrinterInSameFile()
{
	if (already_warned.screen_printer_in_same_file) {
		return;
	}

	PrintWarning("found both screen and printer fonts in the same file");
	already_warned.screen_printer_in_same_file = true;
}

// ***************************************************************************
// CPI file parser - warnings, screen font related
// ***************************************************************************

void CpiParser::WarnScreenFontDataSize(const uint32_t font_offset,
                                       const uint32_t expected, const uint32_t real)
{
	const auto message = format_str("expected screen font data size %d, real size %d",
	                                expected, real);
	PrintWarning(font_offset, message);
}

void CpiParser::WarnFontSizeDuplicateOffsets(const uint32_t struct_offset,
                                             const std::string& struct_name,
                                             const uint8_t height)
{
	const auto message = format_str("multiple offsets found for screen font 8x%d",
	                                height);
	PrintWarning(struct_offset, struct_name, message);
}

void CpiParser::WarnUnknownScreenFontFormat(const uint32_t struct_offset,
                                            const std::string& struct_name,
                                            const uint16_t version)
{
	const auto message = format_str("unknown screen font format %d", version);
	PrintWarning(struct_offset, struct_name, message);
}

void CpiParser::WarnScreenFontWithAspect(const uint32_t struct_offset,
                                         const std::string& struct_name)
{
	PrintWarning(struct_offset,
	             struct_name,
...
void CpiParser::WarnScreenFontWidthInvalid(const uint32_t struct_offset,
                                           const std::string& struct_name,
                                           const uint8_t width)
{
	const auto message = format_str("screen font width %d is invalid", width);
	PrintWarning(struct_offset, struct_name, message);
}

void CpiParser::WarnScreenFontDuplicateSize(const uint32_t struct_offset,
                                            const std::string& struct_name,
                                            const uint8_t height)
{
	const auto message = format_str("screen font size 8x%d found more than once",
	                                height);
	PrintWarning(struct_offset, struct_name, message);
}

void CpiParser::WarnScreenFontWithoutSizes(const uint32_t struct_offset,
                                           const std::string& struct_name)
{
	PrintWarning(struct_offset, struct_name,
	             "screen font does not define any font size");
}

void CpiParser::WarnScreenFontWithoutUsefulSizes(const uint32_t font_offset)
{
	PrintWarning(font_offset,
	             "screen font does not contain any of the 8x16, 8x14 and 8x8 sizes");
}

void CpiParser::WarnScreenFontSizeWithoutCharacters(const uint32_t struct_offset,
                                                    const std::string& struct_name)
{
	PrintWarning(struct_offset, struct_name,
...
void CpiParser::WarnUnknownScreenFontDevice(const uint32_t struct_offset,
                                            const std::string& struct_name,
                                            const std::string& device_name)
{
	const auto message = format_str("unknown screen font device '%s'",
	                                device_name.c_str());
	PrintWarning(struct_offset, struct_name, message);
}

// ***************************************************************************
// CPI file parser - warnings, printer font related
// ***************************************************************************

void CpiParser::WarnPrinterInNonMsDosFile(const uint32_t struct_offset,
                                          const std::string& struct_name)
{
	PrintWarning(struct_offset, struct_name,
...
{
	return (header.device_type == 1) && (header.device_name == DeviceEga);
}

bool CpiParser::IsLcdCodePage(const CodePageEntryHeader& header)
{
	return (header.device_type == 1) && (header.device_name == DeviceLcd);
}
...
bool CpiParser::IsPrinterCodePage(const CodePageEntryHeader& header)
{
	...
	return (header.device_type == 1) &&
	       KnownPrinterDevices.contains(header.device_name);
}
...
void CpiParser::StoreFontType(const CodePageEntryHeader& header, ParserResult& result)
{
	...
	if (found.device_names.size() > 1) {
		WarnMultipleDevicesInSameFile();
	}

	if (found.code_pages.contains({header.device_name, header.code_page})) {
		WarnDuplicatedFont(header.struct_offset,
		                   header.StructName,
		                   header.device_name,
		                   header.code_page);
	} else {
	...
	if (is_screen) {
		...
		if (is_ega) {
			...
			if (IsFormatWinNt()) {
				WarnNonEgaInWinNtFile(header.struct_offset,
				                      header.StructName);
			}
		} else {
		...
		if (!IsFormatMsDos()) {
			WarnPrinterInNonMsDosFile(header.struct_offset,
			                          header.StructName);
		}
	} else {
		WarnUnknownDeviceType(header.struct_offset, header.StructName);
	}

	if (result.found_screen_fonts && result.found_printer_fonts) {
		WarnScreenPrinterInSameFile();
	}
}
                                                          ...
                                                          const uint16_t total_size)
{
	...
	for (size_t idx = 0; idx < num_fonts; ++idx) {
		...
		if (!header->num_chars) {
			WarnScreenFontSizeWithoutCharacters(header->struct_offset,
			                                    header->StructName);
			continue;
		}
		...
		if (header->x_aspect != 0 || header->y_aspect != 0) {
			...
			WarnScreenFontWithAspect(header->struct_offset,
			                         header->StructName);
			should_skip = true;
		}
		...
		if ((height == 16 && !result.font_8x16.empty()) ||
			...
			WarnScreenFontDuplicateSize(header->struct_offset,
			                            header->StructName,
			                            height);
			should_skip = true;
		}
	...
	if (end_offset - start_offset != total_size &&
		...
		WarnScreenFontDataSize(start_offset,
		                       total_size,
	...
	if (result.font_8x16.empty() && result.font_8x14.empty() &&
		...
		WarnScreenFontWithoutUsefulSizes(start_offset);
	}
                                                          ...
                                                          const uint16_t total_size)
{
	...
	for (size_t idx = 0; idx < num_fonts; ++idx) {
		...
		if (width != 8) {
			if (width > 8) {
				WarnScreenFontWidthInvalid(header->struct_offset,
				                           header->StructName,
				                           width);
			}
			continue;
...
std::optional<ScreenFont> CpiParser::GetScreenFont(const uint32_t font_offset)
{
	...
	if (header->num_fonts == 0) {
		WarnScreenFontWithoutSizes(header->struct_offset, header->StructName);
		SetCurrentOffset(stored_offset);
		return {};
	}
	...
	case 0:
		...
		if (IsFormatMsDos()) {
			...
			WarnUnknownScreenFontFormat(header->struct_offset,
			                            header->StructName,
			                            header->version);
		}
		break;
...
ParserResult CpiParser::GetFontsCommonPart(const uint16_t code_page_filter)
{
	...
	auto adapt_offset = [&](const uint32_t value,
	                        bool& is_adaptation_needed) -> uint32_t {
	...
	auto adapt_cpih = [&](const uint32_t value) -> uint32_t {
		return adapt_offset(value, cpih_offset_adaptation_needed);
	};

	auto adapt_cpeh = [&](const uint32_t value) -> uint32_t {
		return adapt_offset(value, cpeh_offset_adaptation_needed);
	};

	auto check_screen_font = [](const std::optional<ScreenFont>& font) {
		if (!font) {
			return false;
		}
		...
		return !font->font_8x16.empty() || !font->font_8x14.empty() ||
		       !font->font_8x8.empty();
	};
	...
	if (font_info_header->num_code_pages == 0) {
		WarnNoCodePages(font_info_header->struct_offset,
	...
	while (true) {
		...
		const auto code_page = header->code_page;

		bool should_skip = false;

		// Check if code page is valid
		if (code_page == 0) {
			WarnInvalidCodePage(header->struct_offset,
			                    header->StructName,
			                    code_page);
			should_skip = true;
		}
		...
		if (!is_device_valid) {
			WarnInvalidDeviceName(header->struct_offset,
			                      header->StructName);
			should_skip = true;
		}
		...
		if (font_offset < GetCurrentOffset()) {
			...
			WarnOffsetPointingBackwards(header->struct_offset,
			                            header->StructName);
			should_skip = true;
		}
		...
		if (!should_skip && is_screen &&
		    IsCodePageMatching(*header, code_page_filter)) {
			const auto font = GetScreenFont(font_offset);
			if (check_screen_font(font)) {
				if (is_ega && !result_ega.contains(code_page)) {
					result_ega[code_page] = *font;
				}
				if (is_lcd && !result_lcd.contains(code_page)) {
					result_lcd[code_page] = *font;
				}
				if (is_screen && !is_ega && !is_lcd &&
				    !result_other.contains(code_page)) {
					result_other[code_page] = *font;
				}
			}
		}
                                      ...
                                      const uint16_t code_page_filter)
{
	...
	if (fih_offset < GetCurrentOffset()) {
		WarnOffsetPointingBackwards(0, FontFileHeader::StructName);
		return false;
	}
                                      ...
                                      const uint16_t code_page_filter)
{
	...
	if (fih_offset != GetCurrentOffset()) {
		...
		WarnStructNotDirectlyAfterPrevious(header->struct_offset,
		                                   header->StructName,
	...
	for (uint16_t idx = 0; idx < header->fonts_per_codepage; ++idx) {
		...
		case 16:
			if (offset_8x16) {
				WarnFontSizeDuplicateOffsets(header->struct_offset,
				                             header->StructName,
				                             16);
			} else {
...
{
	if (id_byte == IdMsDosWinNt) {
		if (id_string == SignatureMsDos) {
			file_format = FileFormat::MsDos;
			return;
		} else if (id_string == SignatureWinNt) {
			file_format = FileFormat::WinNt;
			return;
		}
	} else if (id_byte == IdDrDos && id_string == SignatureDrDos) {
...
ParserResult CpiParser::GetFonts(const uint16_t code_page_filter)
{
	...
	if (HasUpxSignature()) {
		...
		WarnInvalidFileHeader(header->struct_offset, header->StructName);
	}
...
const std::optional<ScreenFont> get_patched_screen_font(const uint16_t code_page)
{
	...
	if (!ega_font_storage.contains(*storage_index)) {
		return {};
	}
