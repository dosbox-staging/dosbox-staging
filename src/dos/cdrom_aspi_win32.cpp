...
bool CDROM_Interface_Aspi::GetAudioTracks(uint8_t& stTrack, uint8_t& endTrack, TMSF& leadOut) 
{
	TOC toc;
...
bool CDROM_Interface_Aspi::GetAudioTrackInfo	(uint8_t track, TMSF& start, unsigned char& attr)
{
	TOC toc;
	if (GetTOC((LPTOC)&toc) == SS_COMP) {
...
bool CDROM_Interface_Aspi::PlayAudioSector(const uint32_t start, uint32_t len)
{
//	SRB_ExecSCSICmd s;
	ASPI_SRB s;
	DWORD dwStatus;
...
bool CDROM_Interface_Aspi::ReadSectors(PhysPt buffer, bool raw, const uint32_t sector, const uint16_t num)
{
	//SRB_ExecSCSICmd s;
	ASPI_SRB s;
	DWORD dwStatus;
	...
	Bitu   buflen	= raw?BYTES_PER_RAW_REDBOOK_FRAME*num:BYTES_PER_COOKED_REDBOOK_FRAME*num;
