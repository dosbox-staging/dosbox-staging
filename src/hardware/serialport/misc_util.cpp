...
NETClientSocket *NETClientSocket::NETClientFactory(SocketTypesE socketType, const char *destination, uint16_t port) {
	switch (socketType) {
...
NETServerSocket *NETServerSocket::NETServerFactory(SocketTypesE socketType, uint16_t port) {
	switch (socketType) {
		case SOCKET_TYPE_TCP:
...
ENETServerSocket::ENETServerSocket(uint16_t port) {
	if (!NetWrapper_InitializeENET()) return;
...
ENETClientSocket::ENETClientSocket(const char *destination, uint16_t port) {
	...
	//***FIX*** This needs to be handled in the general updateState method.  We can't freeze DOSBox for 5 seconds!
		...
		switch (event.type) {
			case ENET_EVENT_TYPE_RECEIVE:
				for (size_t x=0; x<event.packet->dataLength; x++) {
					...
					LOG_INFO("ENET IN:  '%c' %d", event.packet->data[x], event.packet->data[x]);
				}
				enet_packet_destroy(event.packet);
				break;
