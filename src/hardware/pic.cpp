...
static void AddEntry(PICEntry entry)
{
	...
	auto it = std::lower_bound(pic_queue.begin(),
	                           pic_queue.end(),
	                           entry,
	                           [](const PICEntry& a, const PICEntry& b) {
...
{
	pic_queue.erase(std::remove_if(pic_queue.begin(),
	                               pic_queue.end(),
	                               [&](const PICEntry& entry) {
...
void TIMER_DelTickHandler(TIMER_TickHandler handler) {
	ticker_blocks.erase(std::remove(ticker_blocks.begin(), ticker_blocks.end(), handler),
	                    ticker_blocks.end());
}
...
void TIMER_AddTick(void) {
	...
	CPU_Cycles    = 0;
	PIC_Ticks++;
	/* Go through the list of scheduled events and lower their index
	 * with 1.0 */
...
public:
	PIC_8259A(Section* configuration):Module_base(configuration){
		...
		// Empirical testing never saw pic_queue go above 6
		// entries, but add a few more to be safe.
		pic_queue.reserve(16);
		// There are only 12 tickers that can get added in the codebase,
		// but add a few more for future proofing.
		ticker_blocks.reserve(16);
	}
