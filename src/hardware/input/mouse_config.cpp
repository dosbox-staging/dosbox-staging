...
constexpr auto BuiltinDosDriverNoTsr = "notsr";
...
static void config_init(Section_prop& secprop)
{
	...
	prop_str = secprop.Add_string("builtin_dos_mouse_driver", only_at_start, BuiltinDosDriverOn);
	assert(prop_str);
	prop_str->Set_values({BuiltinDosDriverOff,
	                      BuiltinDosDriverOn,
	                      BuiltinDosDriverNoTsr});
	prop_str->Set_help(
	        ...
	        "          doesn't work with your game, try the 'notsr' setting.\n"
	        "  notsr:  Enable the mouse driver without simulating the TSR program. Let us\n"
