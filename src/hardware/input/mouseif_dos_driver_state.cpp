...
std::vector<uint8_t> MouseDriverState::ReadBinaryData() const
{
	...
	if (pt) {
		MEM_BlockRead(PhysicalMake(pt, 0), result.data(), result.size());
	} else {
...
void MouseDriverState::WriteBinaryData(const std::vector<uint8_t> &source)
{
	...
	if (pt) {
		MEM_BlockWrite(PhysicalMake(pt, 0), source.data(), source.size());
	} else {
...
void MouseDriverState::Win386Instance_SetDataPtr(const size_t index, const RealPt value)
{
	switch (index) {
	case 0:
		SET_UINT32(win386_instances.instance0_data_ptr, value);
		return;
	case 1:
		SET_UINT32(win386_instances.instance1_data_ptr, value);
		return;
	default:
		assert(false);
		return;
