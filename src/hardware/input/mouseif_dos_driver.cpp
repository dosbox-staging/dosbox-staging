...
static struct {
	...
	bool is_button_changed = false;
	bool is_wheel_moved    = false;
...
static bool is_immediate_mode()
{
	...
	// entirely compatible with Windows running as a guest system
	return mouse_config.dos_driver_immediate && !is_win386_foreground;
	// XXX test immediate mode in some protected mode software
}

static bool has_pending_event()
{
	return pending.is_mouse_moved ||
	       pending.is_button_changed ||
	       pending.is_wheel_moved;
}

static bool has_pending_event_win386()
{
	MouseDriverState state(*state_segment);
...
static void maybe_trigger_event()
{
	...
	if (!has_pending_event() && !has_pending_event_win386()) { // XXX only one of these!
		return;
	}
...
static void clear_pending_events()
{
	...
	pending.is_mouse_moved    = false;
	pending.is_button_changed = pending.button_state._data;
	pending.is_wheel_moved    = false;
	maybe_start_delay_timer(delay_ms);
}
...
static void restore_cursor_background_gfx()
{
	...
	int16_t x1 = static_cast<int16_t>(state.Background_GetPosX());
	int16_t y1 = static_cast<int16_t>(state.Background_GetPosY());
                                 ...
                                 const float x_abs, const float y_abs)
{
	...
	auto calculate = [](const float absolute,
	...
	// XXX support win386 GFX mode
	const float x = calculate(x_abs,
		is_win386_foreground ? CharToPixelRatio * INT10_GetTextColumns()
		                     : mouse_shared.resolution_x);
	const float y = calculate(y_abs,
		is_win386_foreground ? CharToPixelRatio * INT10_GetTextRows()
...
void MOUSEDOS_NotifyButton(const MouseButtons12S new_buttons_12S)
{
	...
	if (pending.button_state != new_button_state) {
		pending.is_button_changed = true;
...
uint8_t MOUSEDOS_DoInterrupt()
{
	...
	// XXX test that we are not spamming unnecessary interrupts
	if ((!is_win386_foreground && !has_pending_event()) ||
	    (is_win386_foreground  && !has_pending_event_win386())) { // XXX we need one function
		return 0x00;
	}
	...
	if (!is_win386_foreground && pending.is_mouse_moved) {
		mask = update_moved();
	...
	if (!is_win386_foreground && pending.is_wheel_moved) {
...
void MOUSEDOS_FinalizeInterrupt()
{
	...
	if (has_pending_event()) { // XXX also has_pending_event_win386
...
void MOUSEDOS_HandleWindowsCallout()
{
	...
	// Unknown function
	default:
		LOG_WARNING("MOUSE (DOS): Windows callout function 0x%04x not implemented", reg_cx);
		break;
	}
}
...
void MOUSEDOS_NotifyWindowsBackground()
{
	...
	// XXX reset counters/ clear pending events
	is_win386_foreground = false;
}
...
{
	// LOG_ERR("XXX mouse event, e: 0x%04x, b: 0x%04x, x: %d, y: %d", win386_events, win386_buttons, win386_abs_x, win386_abs_y);
	...
	if (win386_event == WinEventMouseMove) {
		...
		MouseButtons12S pending = {};
		pending.left   = (0 != (win386_buttons & WinButtonLeft));
		pending.right  = (0 != (win386_buttons & WinButtonRight));
		pending.middle = (0 != (win386_buttons & WinButtonMiddle));

		state.Win386Pending_SetButtons(pending);
