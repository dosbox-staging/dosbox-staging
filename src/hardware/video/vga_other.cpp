...
static void write_crtc_data_other(io_port_t, io_val_t value, io_width_t)
{
	...
	case 0x06: // Vertical rows
		...
		if (val > 0 && val != vga.other.vdend) {
			vga.other.vdend = val;
			// The default half-frame period delay leads to
			// flickering in the level start zoom effect of
			// Impossible Mission II on Tandy
	...
	case 0x09: // Max scanline
		val &= 0x1f; // VGADOC says bit 0-3 but the MC6845 datasheet
		             // says bit 0-4
	...
	case 0x0C: // Start Address High Register
		// Bit 12 (depending on video mode) and 13 are actually masked
		// too, but so far no need to implement it.
	...
	default:
		LOG(LOG_VGAMISC, LOG_NORMAL)("MC6845:Write %u to illegal index %x",
		                             val,
		                             vga.other.index);
	}
}
static uint8_t read_crtc_data_other(io_port_t, io_width_t)
{
	...
	case 0x03: // Horizontal and vertical sync width
		// hsyncw and vsyncw should only be populated with their lower
		// 4-bits
...
static void write_lightpen(io_port_t port, io_val_t, io_width_t)
{
	...
	case 0x3dc: // Preset lightpen latch
		if (!vga.other.lightpen_triggered) {
			// TODO: this shows at port 3ba/3da bit 1
			vga.other.lightpen_triggered = true;

			const auto timeInFrame = PIC_FullIndex() -
			                         vga.draw.delay.framestart;

			const auto timeInLine = fmod(timeInFrame,
			                             vga.draw.delay.htotal);

			Bitu current_scanline = (Bitu)(timeInFrame /
			                               vga.draw.delay.htotal);

			vga.other.lightpen = (uint16_t)((vga.draw.address_add / 2) *
			                                (current_scanline / 2));
...
static void update_cga16_color_pcjr()
{
	...
	const float pixel_clock_delay = (cp_d * chroma_coefficient +
	                                 rgbi_pixel_delay * rgbi_d) /
	...
	for (uint8_t i = 0; i < 4; i++) {
		...
		for (uint8_t j = 0; j < 6; j++) {
			...
			const auto d = 2.0f * (1.0f - cosf(duty * 2 * tau)) /
			               (2 * tau);

			const auto x = (phases[j] + 21.5f * ns + pixel_clock_delay) /
			                       360.0f +
			               i / 4.0f;

			chroma_signals[j + 1][i] = a + b * sinf(x * tau) +
			                           c * cosf(x * tau) +
			                           d * sinf(x * 2 * tau);
		}
	}
	...
	for (uint8_t x = 0; x < 4; ++x) { // Position of pixel in question
		...
		for (uint8_t bits = 0; bits < (even ? 0x10 : 0x40); ++bits) {
			...
			for (uint8_t p = 0; p < 4; ++p) { // Position within
				                          // color carrier cycle
				...
				const auto chroma = chroma_signals[c][(p + x) & 3] *
				                    chroma_coefficient;
				const auto composite = chroma +
				                       rgbi_coefficients[rgbi];

				Y += composite;
				if (!bw) { // burst on
					I += composite * 2 *
					     cosf(hue_adjust + (p + x) * tau / 4.0f);
					Q += composite * 2 *
					     sinf(hue_adjust + (p + x) * tau / 4.0f);
				}
			}

			Y = clamp(tv_brightness + tv_contrast * Y / 4.0f, 0.0f, 1.0f);
			I = clamp(tv_saturation * tv_contrast * I / 4.0f,
			          -0.5957f,
			          0.5957f);
			Q = clamp(tv_saturation * tv_contrast * Q / 4.0f,
			          -0.5226f,
			          0.5226f);
			...
			auto normalize_and_apply_gamma = [=](float v) -> float {
				const auto normalized = clamp(
				        (v - 0.075f) / (1 - 0.075f), 0.0f, 1.0f);
				return powf(normalized, gamma);
			};

			const auto R = normalize_and_apply_gamma(Y + 0.9563f * I +
			                                         0.6210f * Q);
			const auto G = normalize_and_apply_gamma(Y - 0.2721f * I -
			                                         0.6474f * Q);
			const auto B = normalize_and_apply_gamma(Y - 1.1069f * I +
			                                         1.7046f * Q);
			...
			auto to_linear_rgb = [=](const float v) -> uint8_t {
				...
				// switch to linear RGB space and scale to the
				// 8-bit range
			...
			const auto r = to_linear_rgb(1.5073f * R - 0.3725f * G -
			                             0.0832f * B);

			const auto g = to_linear_rgb(-0.0275f * R +
			                             0.9350f * G + 0.0670f * B);

			const auto b = to_linear_rgb(-0.0272f * R -
			                             0.0401f * G + 1.1677f * B);
...
static void apply_composite_state()
{
	...
	/* 	if (vga.tandy.mode.is_graphics_enabled) {
	                if (is_machine_pcjr()) {
	                        ...
	                        write_cga(0x3d8, vga.tandy.mode.data,
	   io_width_t::byte);
	                }
	        }
	 */
	...
	if (is_machine_tandy()) {
		...
		case M_TANDY4:
			if (vga.tandy.mode_control.is_tandy_640x200_4_color_graphics) {
				VGA_SetCGA4Table( // function sets both medium
				                  // and highres 4color tables
				        vga.attr.palette[0],
				        vga.attr.palette[1],
				        vga.attr.palette[2],
				        vga.attr.palette[3]);
				...
				VGA_SetCGA4Table(
				        vga.attr.palette[vga.tandy.color_select & 0xf],
				        vga.attr.palette[(2 | color_set) &
				                         vga.tandy.palette_mask],
				        vga.attr.palette[(4 | (color_set & r_mask)) &
				                         vga.tandy.palette_mask],
				        vga.attr.palette[(6 | color_set) &
				                         vga.tandy.palette_mask]);
			}
			break;
		default: break;
		...
		case M_TANDY4:
			VGA_SetCGA4Table(vga.attr.palette[0],
			                 vga.attr.palette[1],
			                 vga.attr.palette[2],
			                 vga.attr.palette[3]);
			break;
...
static void write_tandy_reg(uint8_t val)
{
	...
	case 0x0:
		if (is_machine_pcjr()) {
			...
			LOG(LOG_VGAMISC,
			    LOG_NORMAL)("Unhandled Write %2X to tandy reg %X",
			                val,
			                vga.tandy.reg_index);
		}
		break;
	...
	case 0x2: /* Border color */ vga.tandy.border_color = val; break;
	...
	case 0x5: /* Extended ram page register */
		...
		// This is a bit of a hack to enable mapping video memory
		// differently for highres mode
		TandyCheckLineMask();
		VGA_SetupHandlers();
		break;
...
static void write_tandy(io_port_t port, io_val_t value, io_width_t)
{
	...
	case 0x3df:
		...
		// The remapped range is 32kB instead of 16. Therefore CPU_PG
		// bit 0 appears to be ORed with CPU A14 (to preserve some sort
		// of backwards compatibility?), resulting in odd pages being
		// mapped as 2x16kB. Implemeted in vga_memory.cpp Tandy handler.
...
static void write_pcjr(io_port_t port, io_val_t value, io_width_t)
{
	...
	case 0x3df:
		...
		// Bit 2 only has effect when the PCJR upgrade to 128k is
		// installed.
...
void VGA_SetupOther()
{
	...
	if (is_machine_hercules()) {
		...
		vga.draw.font_tables[0] = vga.draw.font_tables[1] = vga.draw.font;
		MAPPER_AddHandler(cycle_hercules_palette,
		                  SDL_SCANCODE_F11,
		                  0,
		                  "hercpal",
		                  "Herc Pal");
	...
	auto register_crtc_port_handlers_at_base = [](const io_port_t base) {
		for (io_port_t i = 0; i < 4; ++i) {
			...
			IO_RegisterWriteHandler(index_port,
			                        write_crtc_index_other,
			                        io_width_t::byte);
			IO_RegisterReadHandler(index_port,
			                       read_crtc_index_other,
			                       io_width_t::byte);

			const auto data_port = check_cast<io_port_t>(index_port + 1);
			IO_RegisterWriteHandler(data_port,
			                        write_crtc_data_other,
			                        io_width_t::byte);
			IO_RegisterReadHandler(data_port,
			                       read_crtc_data_other,
			                       io_width_t::byte);
		}
	};
	...
	if (is_machine_hercules()) {
		...
		// first mode written will be text mode
		vga.herc.mode_control = 0xa;
...
static void init_composite_settings(SectionProp& section)
{
	...
	auto int_prop = section.AddInt("composite_hue",
	                               WhenIdle,
	                               hue.GetDefaultValue());
