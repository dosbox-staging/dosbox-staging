...
public:
	...
	int GetDefaultValue() const { return def; }
...
static void update_cga16_color_pcjr()
{
	...
	const float hue_adjust = (-(90.0f - 33.0f) - hue.AsFloat() + pixel_clock_delay) * tau / 360.0f;
...
static void update_cga16_color()
{
	...
	const auto mode_brightness = brightness.AsFloat() * 5 - 256 * min_v / (max_v - min_v);
	...
	const auto mode_saturation = saturation.AsFloat() * (is_composite_new_era ? 5.8f : 2.9f) / 100;
...
static void PCJr_FindMode()
{
	...
	if (vga.tandy.mode.is_graphics_enabled) {
		if (vga.tandy.mode.is_pcjr_16_color_graphics) {
			...
			// otherwise some 4-colour graphics mode
			const auto new_mode = (cga_comp == CompositeState::On) ? M_CGA16 : M_TANDY4;
			if (vga.mode == M_TANDY16) {
				VGA_SetModeNow(new_mode);
			} else {
				VGA_SetMode(new_mode);
...
static void init_composite_settings(SectionProp& section)
{
	...
	int_prop = section.AddInt("convergence", WhenIdle, convergence.GetDefaultValue());
