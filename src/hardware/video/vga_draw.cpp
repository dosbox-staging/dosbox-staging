...
constexpr auto MaxRowBytes   = RenderMaxWidth * MaxPixelBytes;
...
{
	static int temp[RenderMaxWidth + 10] = {0};
	static int atemp[RenderMaxWidth + 2] = {0};
	static int btemp[RenderMaxWidth + 2] = {0};
...
ImageInfo setup_drawing()
{
	...
	assert(render_height > 0 && render_height <= RenderMaxHeight);
...
void VGA_SetupDrawing(uint32_t /*val*/)
{
	...
	if (previous_video_mode != image_info.video_mode ||
		...
		if (image_info.width > RenderMaxWidth ||
