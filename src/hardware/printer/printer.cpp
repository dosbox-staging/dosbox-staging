...
void Printer::PrintChar(uint8_t ch)
{
	...
	//
	// Skip this for native monospace fonts (Liberation Mono /
	// Courier slot). Their bitmap_left already positions glyphs
	// correctly within the advance cell, and shifting by
	// (cell_px - bitmap.width) / 2 misaligns characters whose ink
	// width varies per glyph — most visibly the CP437 box-drawing
	// characters, where `│`/`─`/`┼` would otherwise land at
	// different X positions and break grids.
	...
	if (!style.prop && act_cpi > 0.0 && !FT_IS_FIXED_WIDTH(cur_font)) {
		...
		const auto glyph_w = static_cast<int>(cur_font->glyph->bitmap.width);
		if (cell_px > glyph_w) {
