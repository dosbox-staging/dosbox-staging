...
//				return (value&0xc0);	// -> 4mb addressable
	...
	bool InitializeRegisters(Bit8u registers[256]) {
		...
		registers[0x05] = 0x02;
		registers[0x06] = 0xA8;	// status register
		registers[0x07] = 0x00;
...
//		LOG_MSG("SST ParseWriteRegister %x:=%x",regnum,value);
	...
	bool InitializeRegisters(Bit8u registers[256]) {
		...
		// reset
		registers[0x04] = 0x00;	// command register (memory space enabled)
		registers[0x05] = 0x00;
		registers[0x06] = 0xA0;	// status register
		registers[0x07] = 0x00;

		registers[0x3c] = 0xff;	// no irq

		// memBaseAddr: size is 16MB
		Bit32u address_space=(((Bit32u)VOODOO_INITIAL_LFB)&0xfffffff0);	// memory space, within first 4GB
