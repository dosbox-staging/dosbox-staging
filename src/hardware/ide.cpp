...
void IDEATAPICDROMDevice::read_toc() {
    ...
    if (Format == 1) { /* Read multisession info */
        ...
        *write++ = (uint8_t)1;        /* @+3 last complete session */
        ...
        LOG_MSG("IDE: ATAPI playing Track %u (attr=0x%02x %02u:%02u:%02u)",first,attr,start.min,start.sec,start.fr);
...
void IDEATAPICDROMDevice::on_atapi_busy_time() {
    ...
    switch (atapi_cmd[0]) {
        case 0x03: /* REQUEST SENSE */
		prepare_read(0, std::min(sense_length,
		                         host_maximum_byte_count));
...
//      int i;
		...
		/* NTS: the way this command works is that the drive reads ONE sector, then fires the IRQ
                ...
                dev->state = IDE_DEV_DATA_READ;
                dev->status = IDE_STATUS_DRQ|IDE_STATUS_DRIVE_READY|IDE_STATUS_DRIVE_SEEK_COMPLETE;
		ata->prepare_read(0, 512 * std::min(ata->multiple_sector_count,
			                            sectcount));
		dev->controller->raise_irq();
                break;
		...
		/* begin another sector */
                ...
                dev->state = IDE_DEV_DATA_WRITE;
                dev->status = IDE_STATUS_DRQ|IDE_STATUS_DRIVE_READY|IDE_STATUS_DRIVE_SEEK_COMPLETE;
		ata->prepare_write(0, 512 * std::min(ata->multiple_sector_count,
			                             sectcount));
		dev->controller->raise_irq();
                break;
            ...
            if ((uint32_t)count != sects || (uint32_t)((drivehead&0xF)+1) != heads) {
                if (count == 0) {
                    ...
                    LOG_MSG("IDE warning: OS is changing logical geometry from C/H/S %u/%u/%u to logical H/S %u/%u/%u",
                        ...
                        (int)ncyls,(drivehead&0xF)+1,(int)count);
...
IDEController::IDEController(Section* configuration,uint8_t index):Module_base(configuration), interface_index(index) {
    ...
    if (index < sizeof(IDE_default_IRQs)) {
        ...
        if (IRQ < 0 || alt_io == 0 || base_io == 0)
            LOG_MSG("IDE: WARNING: IDE interface %u: Insufficient resources assigned by dosbox.conf, and no appropriate default resources for this interface.",index);
    }
}
