...
public:
	...
	CPrinter (uint16_t dpi, uint16_t width, uint16_t height, char* output, bool multipage_output);
	virtual ~CPrinter();
...
private:
	...
	// Process a character that is part of bit image. Must be called iff bit_graph.rem_bytes > 0.
	...
	FT_Library ft_lib = nullptr;			// FreeType2 library used to render the characters
	...
	bool fs_seen = false;				// True if last read character was an FS (0x1C) (IBM commands)

	uint8_t num_param = 0, needed_param = 0;	// Numbers of parameters already read/needed to process command

	uint8_t params[20] = {};				// Buffer for the read params
	uint16_t style = 0;					// Style of font (see STYLE_* constants)
	Real64 cpi = 0.0, act_cpi = 0.0;		// CPI value set by program and the actual one (taking in account font types)
	uint8_t score = 0;					// Score for lines (see SCORE_* constants)

	Real64 top_margin = 0.0, bottom_margin = 0.0, right_margin = 0.0, left_margin = 0.0;	// Margins of the page (in inch)
	Real64 page_width = 0.0, page_height = 0.0;								// Size of page (in inch)
	Real64 default_page_width = 0.0, default_page_height = 0.0;					// Default size of page (in inch)
	...
	Real64 extra_intra_space = 0.0;		// Extra space between two characters (set by program, in inch)

	bool char_read = false;				// True if a character was read since the printer was last initialized
	bool auto_feed = false;				// True if a LF should automatically added after a CR
	bool print_upper_contr = false;		// True if the upper command characters should be printed

	struct BitGraphicParams				// Holds information about printing bit images
	{
		uint16_t horiz_dens = 0, vert_dens = 0;	// Density of image to print (in dpi)
		...
		uint8_t read_bytes_column = 0;		// Bytes read so far for the current column
	} bit_graph;
	...
	Real64 defined_unit = -1.0;			// Unit used by some ESC/P2 commands (negative => use default)
	...
	uint16_t num_print_as_char = 0;			// Number of bytes to print as characters (even when normally control codes)

	char* output = nullptr;				// Output method selected by user
	void* output_handle = nullptr;		// If not null, additional pages will be appended to the given handle
	bool multipage_output = false;		// If true, all pages are combined to one file/print job etc. until the "eject page" button is pressed
