...
static void VGA_VerticalTimer(Bitu /*val*/) {
	...
	if (vga.draw.override || !RENDER_StartUpdate())
		return;
...
void VGA_KillDrawing(void) {
	...
	vga.draw.lines_done = ~0;
	if (!vga.draw.override) RENDER_EndUpdate(true);
}

void VGA_SetOverride(bool override) {
	if (vga.draw.override!=override) {
		
		if (override) {
			VGA_KillDrawing();
			vga.draw.override=true;
		} else {
			vga.draw.override=false;
