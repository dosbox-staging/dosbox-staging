...
// DOS IPX driver was limited to 150 open sockets
#define SOCKTABLESIZE 150
...
// IPAddress for client connection to server
IPaddress ipxServConnIp;

// Incoming packet buffer
uint8_t recvBuffer[IPXBUFFERSIZE];
...
// Linked list of ECB's
ECBClass* ECBList;

// ECBs waiting to be ESR notified
ECBClass* ESRList;
          ...
          buflen(0)
{
	...
	LOG_IPX("ECB: SN%7d created.   Number of ECBs: %3d, ESR %4x:%4x, ECB %4x:%4x",
	        SerialNumber,
	        ECBAmount,
	        real_readw(RealSegment(ECBAddr), RealOffset(ECBAddr) + 6),
	        real_readw(RealSegment(ECBAddr), RealOffset(ECBAddr) + 4),
	        segment,
	        offset);
#endif
...
void ECBClass::NotifyESR(void)
{
	...
	if (ESRval || databuffer) {
		// databuffer: write data at realmode/v86 time
		//
...
{
	for (Bitu i = 0; i < 6; i++) {
		real_writeb(RealSegment(ECBAddr),
		            RealOffset(ECBAddr) + 28 + i,
		            immAddr[i]);
	}
}
...
{
	for (Bitu i = 0; i < 6; i++) {
		immAddr[i] = real_readb(RealSegment(ECBAddr),
		                        RealOffset(ECBAddr) + 28 + i);
	}
}
...
static void OpenSocket(void)
{
	...
	if (sockNum == 0x0000) {
		...
		if (sockAlloc > 0x7fff) {
			// I have no idea how this could happen if the IPX
			// driver is limited to 150 open sockets at a time
...
static void IPX_AES_EventHandler(uint32_t param)
{
	...
	while (tmpECB != nullptr) {
		...
		if (tmpECB->iuflag == USEFLAG_AESCOUNT &&
		    param == (Bitu)tmpECB->ECBAddr) {

			tmpECB->setCompletionFlag(COMP_SUCCESS);
			tmpECB->setInUseFlag(USEFLAG_AVAILABLE);
			tmpECB->NotifyESR();
			// LOG_IPX("AES Notification: ECB S/N
			// %d",tmpECB->SerialNumber);
			return;
		}
...
static void handleIpxRequest(void)
{
	...
	case 0x0008: // Get interval marker
		// BIOS_TIMER
		reg_ax = mem_readw(0x46c);
		break;

	case 0x0009: // Get internetwork address
	{
		LOG_IPX("IPX: Get internetwork address %2x:%2x:%2x:%2x:%2x:%2x",
		        localIpxAddr.netnode[5],
		        localIpxAddr.netnode[4],
		        localIpxAddr.netnode[3],
		        localIpxAddr.netnode[2],
		        localIpxAddr.netnode[1],
		        localIpxAddr.netnode[0]);
	...
	case 0x000d: // get packet size
		// retry count
		reg_cx = 0;
		// real implementation returns 1024
		reg_ax = 1024;
		break;
	...
	case 0x001a: // get driver maximum packet size
		// retry count
		reg_cx = 0;
		// max packet size: something near the ethernet packet size
		reg_ax = IPXBUFFERSIZE;
		break;
...
static void pingAck(IPaddress retAddr)
{
	...
	memcpy(regHeader.src.addr.byNode.node,
	       localIpxAddr.netnode,
...
static void sendPacket(ECBClass* sendecb)
{
	...
	for (i = 0; i < fragCount; i++) {
		...
		for (t = 0; t < tmpFrag.size; t++) {
			outbuffer[packetsize] = real_readb(tmpFrag.segment,
			                                   tmpFrag.offset + t);
			packetsize++;
	...
	// Real implementation uses the ImmedAddr to check wether this is a
	// broadcast
	...
	if (isloopback || islocalbroadcast) {
		...
		LOG_IPX("Packet back: loopback:%d, broadcast:%d",
		        isloopback,
		        islocalbroadcast);
	}
	...
	void Run(void) override
	{
		...
		if (cmd->FindCommand(1, temp_line)) {
			...
			if (strcasecmp("startserver", temp_line.c_str()) == 0) {
				if (!isIpxServer) {
					...
					if (!cmd->FindCommand(2, temp_line)) {
						...
						udpPort = strtol(temp_line.c_str(),
						                 nullptr,
						                 10);
					}

					startsuccess = IPX_StartServer(
					        (uint16_t)udpPort);
			...
			if (strcasecmp("disconnect", temp_line.c_str()) == 0) {
				...
				// TODO: Send a packet to the server notifying
				// of disconnect
...
Bitu IPX_ESRHandler(void)
{
	...
	while (ESRList != nullptr) {
		...
		delete ESRList; // Destructor updates this pointer to the next
		                // value or NULL
	} // while
...
private:
	...
	// the constructor did not take the disabled early-return). The
	// destructor must tear down based on what this instance installed, not
	// on the live 'ipx' setting, which may already reflect a pending
	// runtime change.
...
public:
	IPX(SectionProp& _section) : section(_section)
	{
		...
		if (!dospage) {
			dospage = DOS_GetMemory(2); // can not be freed yet
		}

		PhysPt phyDospage = PhysicalMake(dospage, 0);

		LOG_IPX("ESR callback address: %x, HandlerID %d",
		        phyDospage,
		        call_ipxesr1);
		...
		phys_writeb(phyDospage + 9, (uint8_t)0x38); // Extra Callback
		                                            // instruction
		...
		phys_writeb(phyDospage + 19, (uint8_t)0xCF); // IRET: restores
		                                             // flags, CS, IP
