                                   ...
                                   [[maybe_unused]] int total_bytes)
{
	...
	// full level and these transitions don't bleed into captured WAV / AVI
	// audio.
	//
	...
	// audio, the buffer is silence-filled by the shortfall path above.
	...
	// full gain -- click. Hold the ramp at its current value until we see
	// real samples. Fade-out has no such gating: when there's nothing to
	// drain, gain just settles at 0 against whatever silence the mixer
	// thread pushed, which is the desired end state.
...
// mixer_callback applies its gain ramp to those samples before they reach the
// host.
