...
static void set_mixer_state(const MixerState new_state)
{
	...
	if (new_state == MixerState::Muted || new_state == MixerState::Paused) {
		// Clear queued audio so resume doesn't replay pre-pause
		// samples. capture_queue is NOT cleared -- it's a separate path
		// for video capture that must drain in FIFO order for
