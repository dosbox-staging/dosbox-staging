...
struct MixerSettings {
	...
	//               non-Audible, `mix_samples()` still runs (so the
	//               capture queue IS fed at full level) and the fade
	//               below ramps the SDL-bound `output_buffer` toward
	//               zero before it reaches `final_output`.
	//
	// Pause is queried directly via `DOSBOX_IsPaused()` /
	// `DOSBOX_IsPauseRequested()`. The fade-out starts at the
	// pause-request edge (via `DOSBOX_IsPauseRequested()`) so it has
	// real audio to attenuate against. The mixer thread's silence
	// short-circuit and the capture skip only trip when subsystems are
	// actually paused (via `DOSBOX_IsPaused()`).
	//
	bool no_sound                          = false;
...
// Silence-edge smoothing ramp. Ramps the SDL-bound `output_buffer` between
// full and muted over a few ms to mask the step discontinuity that would
// otherwise click when a non-zero sample is followed by 0.0 (entering
// pause / mute) or the reverse (leaving them).
//
// Applied here in the mixer thread, AFTER `mix_samples()` has fed the
// capture queue at full level -- so this fade never bleeds into captured
// WAV / AVI audio.
//
// Uses `DOSBOX_IsPauseRequested()` (not `DOSBOX_IsPaused()`) so the
// fade-out starts at the pause-request edge, not at the actual pause.
// During the pending window `mix_samples()` still runs and produces real
// audio -- that's what the fade attenuates against, so it doesn't hit
// silence half-way through and click.
//
...
static void mixer_thread_loop()
{
	...
	while (!mixer.thread_should_quit) {
		...
		// capture invariant). At this point the fade-out has already
		// completed (that's what triggered the Pending -> Paused
		// transition), so `playback_gain` is already at 0 and no
		// further ramp is needed here.
		if (DOSBOX_IsPaused()) {
			lock.unlock();
