...
PluginAndModel try_load_plugin(const SoundCanvasModel model)
{
...
PluginAndModel load_model(const std::string& wanted_model_name)
{
	...
	for (const auto& model : candidate_models) {
		if (load_first_available || wanted_model_name == model->config_name) {
			if (auto p = try_load_plugin(model->model); p.plugin) {
				return p;
			}
		}
	}
...
SoundCanvasModel MidiDeviceSoundCanvas::GetModel() const {
	assert(active_model);
	return *active_model;
}
...
void SOUNDCANVAS_ListDevices(MidiDeviceSoundCanvas* device, Program* caller)
{
	...
	const auto available_models = [&] {
		...
		for (auto m : all_models) {
			if (const auto p = try_load_plugin(m->model); p.plugin) {
	...
	const std::optional<SoundCanvasModel> active_model = [&] {
		std::optional<SoundCanvasModel> model = {};
		if (device) {
