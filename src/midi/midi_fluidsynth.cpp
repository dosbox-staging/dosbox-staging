	...
	if (!sf_dir.empty()) {
		...
		if (path_exists(sf_dir)) {
			...
			NOTIFY_DisplayWarning(Notification::Source::Console,
			                      "FSYNTH",
			                      "Invalid 'soundfont_dir' setting, cannot open "
			                      "directory [color=white]'%s'[reset]; using ''",
			                      sf_dir.c_str());
                              ...
                              const double max_val)
{
	...
	if (val < min_val || val > max_val) {
		NOTIFY_DisplayWarning(Notification::Source::Console,
		                      "FSYNTH",
		                      "Invalid '%s' setting: [color=white]%s[reset]; "
		                      "needs to be between %.2f and %.2f; "
		                      "using default (%.2f)",
		                      name,
		                      str_val.c_str(),
		                      min_val,
...
MidiDeviceFluidSynth::MidiDeviceFluidSynth()
{
	...
	if (!fluidsynth_channel->TryParseAndSetCustomFilter(filter_prefs)) {
		if (filter_prefs != "off") {
			NOTIFY_DisplayWarning(Notification::Source::Console,
			                      "FSYNTH",
			                      "Invalid 'fsynth_filter' value: "
								  "[color=white]'%s'[reset], using 'off'",
			                      filter_prefs.c_str());
		}
