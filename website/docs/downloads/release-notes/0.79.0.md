...
<figure markdown>
  <video controls width=760>
    <source src="../videos/alone-in-the-dark-intro.webm" type="video/webm; codecs=vp9,opus">
    <source src="../videos/alone-in-the-dark-intro.mp4" type="video/mp4">
    Your browser does not support the <code>video</code> element.
  </video>
  <figcaption>Alone in the Dark (1992, floppy version) &mdash; introduction</figcaption>
</figure>
...
| --------------------------------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| Alone in the Dark<br>(floppy version)         | OPL2           | <audio controls src="../audio/alone-in-the-dark.mp3"> Your browser does not support the <code>audio</code> element. </audio>   |
| Warcraft: Orcs & Humans                       | OPL3           | <audio controls src="../audio/warcraft.mp3"> Your browser does not support the <code>audio</code> element. </audio>            |
| Dungeon Master II:<br>The Legend of Skullkeep | Dual&nbsp;OPL2 | <audio controls src="../audio/dungeon-master-ii.mp3"> Your browser does not support the <code>audio</code> element. </audio>   |
| Ultima Underworld I: The Stygian Abyss        | Dual OPL2      | <audio controls src="../audio/ultima-underworld-i.mp3"> Your browser does not support the <code>audio</code> element. </audio> |
| Elvira: Mistress of the Dark                  | Dual&nbsp;OPL2 | <audio controls src="../audio/elvira.mp3"> Your browser does not support the <code>audio</code> element. </audio>              |
| Skate or Die!                                 | Tandy          | <audio controls src="../audio/skate-or-die.mp3"> Your browser does not support the <code>audio</code> element. </audio>        |
| Zak McKracken and the Alien Mindbenders       | Tandy          | <audio controls src="../audio/zak.mp3"> Your browser does not support the <code>audio</code> element. </audio>                 |
| Budokan: The Martial Spirit                   | CMS            | <audio controls src="../audio/budokan.mp3"> Your browser does not support the <code>audio</code> element. </audio>             |
| The Secret of Monkey Island                   | CMS            | <audio controls src="../audio/monkey-island.mp3"> Your browser does not support the <code>audio</code> element. </audio>       |
...
!!! note
    In the SF2 format the reverb and chorus amounts are specified on a per-instrument basis, while the mixer-level effects are applied "globally" to all instruments in the summed FluidSynth output. Therefore, what works best really depends on the particular SoundFont in question and on individual user preference (see [this discussion](https://github.com/dosbox-staging/dosbox-staging/pull/1814) for further technical details).
...
<figure markdown>
  <video controls width=760>
    <source src="../videos/pcspeaker_impulse_wizball.webm" type="video/webm; codecs=vp9,opus">
    <source src="../videos/pcspeaker_impulse_wizball.mp4" type="video/mp4">
    Your browser does not support the <code>video</code> element.
  </video>
  <figcaption>Wizball (1987) &mdash; using <code>discrete</code> PC speaker emulation</figcaption>
</figure>
...
| ---------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `disney` | <audio controls src="../audio/modplay_pkunk_lpt_dac_disney.flac"> Your browser does not support the <code>audio</code> element. </audio> |
| `covox`  | <audio controls src="../audio/modplay_pkunk_lpt_dac_covox.flac"> Your browser does not support the <code>audio</code> element. </audio>  |
| `ston1`  | <audio controls src="../audio/modplay_pkunk_lpt_dac_ston1.flac"> Your browser does not support the <code>audio</code> element. </audio>  |
...
| default | tandy-warm |
| --------| ---------- |
| ![Defender of the Crown (default)](images/dotc-default.png){ loading=lazy } | ![Defender of the Crown (tandy-warm)](images/dotc-tandy-warm.png){ loading=lazy } |
...
| --------| ------------ |
| ![Gold Rush! (default)](images/gold-rush-default.png){ loading=lazy } | ![Gold Rush! (agi-amigaish)](images/gold-rush-agi-amigaish.png){ loading=lazy } |
...
| default | colodore |
| ------- | -------- |
| ![Zak McKracken (default)](images/zak-default.png){ loading=lazy } | ![Zak McKracken (colodore)](images/zak-colodore.png){ loading=lazy } |
...
| Good | Bad |
| -----| --- |
| ![Timequest (good)](images/timequest-good.png){ loading=lazy } | ![Timequest (bad)](images/timequest-bad.png){ loading=lazy } |
...
| Good | Bad |
| ---- | --- |
| ![Space Quest III (good)](images/sq3-good.png){ loading=lazy } | ![Space Quest III (bad)](images/sq3-bad.png){ loading=lazy } |
...
| Good | Bad |
| ---- | --- |
| ![Windwalker (good)](images/windwalker-good.png){ loading=lazy } | ![Windwalker (bad)](images/windwalker-bad.png){ loading=lazy } |
...
Due to the way macOS handles DPI scaling, the image was drawn at a much lower resolution, usually just a quarter of the window size. Then it was stretched by the system, producing an unpleasantly mushy presentation. But we want our pixels to be so sharp they could cut one's eyes :sunglasses: :dagger:

| Before | After |
| ------ | ----- |
| ![DOSBox Staging on macOS without hiDPI support](images/non_hidpi_win31.png){ loading=lazy } | ![DOSBox Staging on macOS with hiDPI support](images/hidpi_win31.png){ loading=lazy } |
...
![Transparent window](images/transparenct_window.png){ loading=lazy }
...
<figure markdown>
  ![Earthworm Jim (synced CFR)](images/synced_cfr_ewj.png){ loading=lazy }

  <figcaption>Synced CFR mode selected due to vsync-enforced video drivers</figcaption>
</figure>
...
![ATTRIB command help](images/attrib_help.png){ loading=lazy }
...
![Mount support for Physical CD](images/mount_cd.png){ loading=lazy }
...
![Mount images with wildcard](images/imgmount_wildcard.png){ loading=lazy }
...
- `KEYB cz489 867` --- a so-called programmer keyboard layout (national characters available with ++ralt+0++ - ++9++) for Czech and Slovak languages, with Kamenický character encoding - which was more popular than code page 852
...
| ---------------------- | -------------------- |
| ![DIR command (CF)](images/country_code_2.png){ loading=lazy } | ![DIR command (US)](images/country_code_1.png){ loading=lazy } |
...
- [VBADOS](https://git.javispedro.com/cgit/vbados.git/about/) by *javispedro* --- this one requires a corresponding DOS mouse driver to be started; this driver also provides seamless mouse integration for a real DOS if booted inside DOSBox Staging

- [vmwmouse](https://github.com/NattyNarwhal/vmwmouse) by *NattyNarwhal* --- does not require any specific DOS driver and has experimental support for mouse wheel, but current version 0.1 has stability problems
...
<video controls width=760>
  <source src="../videos/joystick_calibration_hotkey_demo.webm" type="video/webm; codecs=vp9,opus">
  <source src="../videos/joystick_calibration_hotkey_demo.mp4" type="video/mp4">
  Your browser does not support the <code>video</code> element.
</video>
...
| ------------ | ------------------------ |
| ![Windows installer welome page](images/windows_installer_start.png){ loading=lazy } | ![Icons after installation](images/icons_after_installation.png){ loading=lazy } |
...
DOS's emulated main memory (up to 64 MiB) and the video card's memory (to up 8 MiB) are both now allocated statically to reduce pressure on heap storage ([#1714](https://github.com/dosbox-staging/dosbox-staging/pull/1714)).

More progress has been made in reducing the use of large variables type-sizes down to purpose-fit sizes per the DOS/hardware specification (typically 8 or 16-bit) ([#1721](https://github.com/dosbox-staging/dosbox-staging/pull/1721)).
...
<figure markdown>
  ![tracy](images/tracy.png){ loading=lazy }
  <figcaption>Tracy runtime-profiling DOSBox Staging</figcaption>
</figure>



It can be enabled at build-time with `-Dtracy=true`. Learn more about it, and how to build the server-side component here: [#1749](https://github.com/dosbox-staging/dosbox-staging/pull/1749).
...
![Coverity metrics)](images/coverity_numbers_2022_09.png){ loading=lazy }

![Coverity graph)](images/coverity_graph_2022_09.png){ loading=lazy }
