...
The exact settings used for these recordings are documented [on the
wiki](https://github.com/dosbox-staging/dosbox-staging/wiki/Audio-configuration-recommendations),
...
- The `MIXER` command has received an overhaul too. First of all, the status
  ...
  - The sample rate column has been removed (it served no useful purpose, and
    the displayed values were often technically correct but misleading).
...
DOS's emulated main memory (up to 64 MiB) and the video card's memory (to up 8
MiB) are both now allocated statically to reduce pressure on heap storage
([#1714](https://github.com/dosbox-staging/dosbox-staging/pull/1714)).
...
16-bit) ([#1721](https://github.com/dosbox-staging/dosbox-staging/pull/1721)).
...
how to build the server-side component here:
[#1749](https://github.com/dosbox-staging/dosbox-staging/pull/1749).
...
- [Fixed](https://github.com/dosbox-staging/dosbox-staging/issues/1722)
  **Ultima Underworld** issues with gaming (high polling rate) mice
