...
`cdrom1.iso`; our game folder has the full name of the game anyway. 
...
!!! danger "Always mount CD-ROM images with `imgmount`"
    ...
    For a trouble-free life, always just use the built-in `imgmount` command.
    There's nothing to be gained from using these 3rd party mounting tools for
    DOS games.
...
turns out it just contains some legal notice we don't really care about.
...
any key or wait a few seconds to progress to the second screen where you'll need
...
but it's not too hard to figure it out either. Reading the manual or some trial and
error might help too.
...
The Sound Blaster card and PC speaker are enabled by default, that's why their
channels appear in the mixer, and the `CDAUDIO` channel is added automatically
whenever we mount a CD-ROM image (as it might contain audio tracks).
...
could shake a stick at, many of them have quite different default volume
...
the volume levels of the Sound Blaster card's internal mixer, but we can
...
you *did* watch the intro video (which I told you to skip, but no hard
feelings), you must have heard similar random audio stutterings from time to
time.
...
sound card. They were the Cadillacs of DOS gaming audio for a while, they were
priced accordingly, and they still sound excellent even by today's standards.
...
and sound effects *with* speech via the Sound Blaster---the computer says no,
buddy.
...
appear vertically stretched on-screen. In fact, they appear _exactly 20% taller_
than they should (just trust me on that for a moment).
...
similary to EGA monitors, and they were a lot less sharp. While this wasn't
...
<figure markdown>
  <a class="glightbox" href="https://archive.org/download/dosbox-staging-v0.81.0-bass/bass-amiga.jpg" >
    ![Beneath a Steel Sky from a parellel universe running on an Amiga in 256 colour mode](https://archive.org/download/dosbox-staging-v0.81.0-bass/bass-amiga-small.jpg){ loading=lazy .skip-lightbox }
  </a>
...
output by default; that's undesirable as the MT-32 has its own built-in reverb
and chorus, so DOSBox Staging is intelligent enough not to apply these effects
twice on its output.
