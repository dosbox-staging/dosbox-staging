...
### CGA on a monochrome monitor

`machine = cga_mono`

Emulates a CGA adapter connected to a monochrome monitor. This produces CGA
video modes rendered in monochrome, with selectable phosphor palettes just
like Hercules mode. A few programs were designed for this specific
configuration.
...
IBM's Color Graphics Adapter (1981) was the first colour graphics option for
the PC. It offered 320&times;200 at 4 colours from fixed palettes, 640&times;200
at 2 colours, and both 40- and 80-column colour text modes.
...
The IBM PCjr (1983) was IBM's short-lived home computer. It extended CGA with
additional video modes, most notably 16-colour 320&times;200 --- a significant
step up from CGA's 4-colour limit at the same resolution. It also included a
TI SN76496 3-voice sound chip, the first built-in sound hardware beyond the PC
speaker on an IBM PC platform.

The PCjr was discontinued after just one year, but its enhanced graphics and
sound live on in the Tandy 1000 line which cloned them. Setting
`machine = pcjr` also enables [PCjr sound](../sound/sound-devices/tandy.md)
and [composite video](composite-video.md) emulation.
...
The CGA, PCjr, and Tandy adapters all support
[composite video emulation](composite-video.md). On real hardware, connecting
these cards to an NTSC composite monitor or TV produced "artifact colours"
--- interference patterns in the analogue signal that create far more colours
than the digital output. Game developers at Sierra On-Line and elsewhere
deliberately exploited this effect.
...
essential for playing early Sierra AGI games (King's Quest, Space Quest) as
the artists intended. See [Composite video](composite-video.md) for
...
IBM's Enhanced Graphics Adapter (1984) was a major step forward: 16
simultaneous colours from a 64-colour palette at up to 640&times;350
resolution. DOSBox emulates EGA with a fixed 256 KB of video memory.
...
IBM's Video Graphics Array (1987) was the last IBM graphics standard to which
the majority of PC clone manufacturers conformed. VGA introduced the iconic
Mode 13h --- 256 colours from a palette of 262,144 at 320&times;200 --- which
became *the* standard DOS gaming mode from the late 1980s through the
mid-1990s. VGA also introduced analog video output, replacing the digital RGBI
signalling of earlier standards.
...
!!! note
    ...
    A small number of games and demos rely on per-scanline VGA register tricks
    (e.g., Lemmings, Pinball Fantasies, Alien Carnage). These may need the
    `svga_paradise` machine type, which is the closest to IBM's original VGA
...
ranging from early 1980s designs to the mid-1990s S3 Trio64.
...
The Paradise PVGA1A (1988) was an early SVGA chipset. It has no VESA VBE
support and defaults to 512 KB of video memory (configurable to 256 KB or
1 MB via [`vmemsize`](../system/general.md#vmemsize)).

This is the closest DOSBox Staging offers to a plain IBM VGA adapter. Use it
for games or demos that expect strict VGA behaviour with no SVGA extensions.
...
The Tseng Labs ET3000 (1987) was one of the earliest SVGA chipsets. Extended
modes include 800&times;600 at 16 colours and 640&times;480 at 256 colours.
No VESA VBE support. Fixed 512 KB video memory.

A niche option for titles or demoscene productions written specifically for
this chipset.
...
The Tseng Labs ET4000 (1989) was one of the most popular SVGA chipsets of the
early 1990s. It offers a wider range of extended modes than the ET3000 and
defaults to 1 MB of video memory (configurable to 256 KB or 512 KB via
[`vmemsize`](../system/general.md#vmemsize)). No VESA VBE support.

Some demoscene productions and a handful of games were written specifically
for the ET4000.
...
The S3 Trio64 (1994) is the default and most capable adapter. It provides VESA
VBE 2.0 support with a linear framebuffer, defaults to 4 MB of video memory
(configurable from 512 KB to 8 MB via
[`vmemsize`](../system/general.md#vmemsize)), and handles everything from
standard VGA up to high-resolution SVGA modes including 1600&times;1200.

This is the right choice for the vast majority of DOS games. Use the
...
| `machine` value  | Adapter                                        | Year | Colours              | Max resolution         |
|------------------|------------------------------------------------|------|----------------------|------------------------|
| `hercules`       | Hercules HGC                                   | 1982 | Mono                 | 720&times;348          |
| `cga_mono`       | CGA (mono monitor)                             | 1981 | Mono                 | 640&times;200          |
| `cga`            | IBM CGA                                        | 1981 | 4 (16&nbsp;composite)| 640&times;200          |
| `pcjr`           | IBM PCjr                                       | 1983 | 16                   | 640&times;200          |
| `tandy`          | Tandy 1000                                     | 1984 | 16                   | 640&times;200          |
| `ega`            | IBM EGA                                        | 1984 | 16 of 64             | 640&times;350          |
| `svga_paradise`  | Paradise PVGA1A                                | 1988 | 256                  | VGA modes              |
| `svga_et3000`    | Tseng Labs ET3000                              | 1987 | 256                  | 800&times;600 (16-col) |
| `svga_et4000`    | Tseng Labs ET4000                              | 1989 | 256+                 | varies by VRAM         |
| `svga_s3`        | S3 Trio64, VBE 2.0 *default*{ .default }       | 1994 | up to 16.7M          | up to 1600&times;1200  |
| `vesa_oldvbe`    | S3 Trio64, VBE 1.2                             | 1994 | up to 16.7M          | up to 1600&times;1200  |
| `vesa_nolfb`     | S3 Trio64, VBE 2.0 (no&nbsp;LFB)              | 1994 | up to 16.7M          | up to 1600&times;1200  |
...
DOSBox Staging also emulates two add-in cards that work alongside the primary
video adapter:
