...
exactly the same way. There was absolutely no reason for preferring `openglnb`
over `opengl`.
...
The `texturenb` output mode continues to be available as a last resort
fallback for weak hardware with no functional OpenGL drivers.
...
- Fixed the hardware cursor handling on the S3 Trio64 card; this fixes the display
  corruption on exit isue in the [Teddybear's Revenge](https://www.pouet.net/prod.php?which=76)
  demo by Eclipse.

- Implemented the missing S3 Trio64 XGA compare and fill routines. This
  lets you use the version 1.70.04 of the S3 Trio64 drivers in Windows 3.1x
...
- Now we use linear 128 KB video memory pages for all VESA modes. Some games exploit
  ...
  single contiguous 128 KB memory block, instead of having to manually flip between two
  64 KB pages. This enhancement gets rid of the odd horizontal black lines in
  the following Synthetic Dimensions games, and possibly in other games and
...
emulating **ESS Enhanced FM Audio**, also known as **ESFM**, the
OPL3-compatible FM synth found on later ESS AudioDrive cards.

In "legacy mode", ESFM is fully compatible with the Yamaha OPL3, and on most
materials, it yields almost identical output (with the occasional subtle
difference). But what really sets it apart is its "native mode" in which it
...
| Game                                          | Audio example                                                                                                                 |
| --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Heroes of Might and Magic II — Title (ESFM)   | <audio controls src="https://dosbox-staging.github.io/static/audio/release-notes/0.81.1/heroes2-title-esfm.mp3"> Your browser does not support the <code>audio</code> element.</audio>   |
| Heroes of Might and Magic II — Title (OPL)  | <audio controls src="https://dosbox-staging.github.io/static/audio/release-notes/0.81.1/heroes2-title-opl.mp3"> Your browser does not support the <code>audio</code> element.</audio>   |
| Heroes of Might and Magic II — In-game (ESFM)      | <audio controls src="https://dosbox-staging.github.io/static/audio/release-notes/0.81.1/heroes2-ingame-esfm.mp3"> Your browser does not support the <code>audio</code> element.</audio>   |
| Heroes of Might and Magic II — In-game (OPL)         | <audio controls src="https://dosbox-staging.github.io/static/audio/release-notes/0.81.1/heroes2-ingame-opl.mp3"> Your browser does not support the <code>audio</code> element.</audio>   |
| The Settlers 2 — Track 5 (ESFM)   | <audio controls src="https://dosbox-staging.github.io/static/audio/release-notes/0.81.1/settlers2-track5-esfm.mp3"> Your browser does not support the <code>audio</code> element.</audio>   |
| The Settlers 2 — Track 5 (OPL)    | <audio controls src="https://dosbox-staging.github.io/static/audio/release-notes/0.81.1/settlers2-track5-opl.mp3"> Your browser does not support the <code>audio</code> element.</audio>   |
| The Settlers 2 — Track 14 (ESFM)    | <audio controls src="https://dosbox-staging.github.io/static/audio/release-notes/0.81.1/settlers2-track14-esfm.mp3"> Your browser does not support the <code>audio</code> element.</audio>   |
| The Settlers 2 — Track 14 (OPL)    | <audio controls src="https://dosbox-staging.github.io/static/audio/release-notes/0.81.1/settlers2-track14-opl.mp3"> Your browser does not support the <code>audio</code> element.</audio>   |
| The Gene Machine (ESFM)  | <audio controls src="https://dosbox-staging.github.io/static/audio/release-notes/0.81.1/the-gene-machine-esfm.mp3"> Your browser does not support the <code>audio</code> element.</audio>   |
| The Gene Machine (OPL)  | <audio controls src="https://dosbox-staging.github.io/static/audio/release-notes/0.81.1/the-gene-machine-opl.mp3"> Your browser does not support the <code>audio</code> element.</audio>   |
| Heaven's Dawn (ESFM)  | <audio controls src="https://dosbox-staging.github.io/static/audio/release-notes/0.81.1/heavens-dawn-esfm.mp3"> Your browser does not support the <code>audio</code> element.</audio>   |
| Heaven's Dawn (OP)  | <audio controls src="https://dosbox-staging.github.io/static/audio/release-notes/0.81.1/heavens-dawn-opl.mp3"> Your browser does not support the <code>audio</code> element.</audio>   |
...
The more adventurous among you can "retrofit" the `ESFM.MID` driver from Miles
Sound System games that have it into earlier ones that don't. 

Here's how **Discworld** sounds with the ESFM driver from **Heaven's
Dawn**—it's surprisingly good!

|                                           | |
| --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Discworld (ESFM)   | <audio controls src="https://dosbox-staging.github.io/static/audio/release-notes/0.81.1/discworld-kitchen-esfm.mp3"> Your browser does not support the <code>audio</code> element.</audio>   |
| Discworld (OPL)  | <audio controls src="https://dosbox-staging.github.io/static/audio/release-notes/0.81.1/discworld-kitchen-opl.mp3"> Your browser does not support the <code>audio</code> element.</audio>   |
...
We've introduced a new `opl_remove_dc_bias` config setting to to `[sblaster]`
section which eliminates these annoying pops in affected games.
...
- The new `cms` config setting in the `[sblaster]` section controls whether to
  emulate the CMS or not. Valid values are `on` and `off`.

!!! warning

    As per usual, `oplmode = cms` is still accepted with a deprecation
    warning, but you should update your configs to use the new model as we
    will eventually remove this backward compatibility option in the future.


### Sound Blaster Pro 2 output filter fix

Previously, toggling the analog output filter programmatically on the Sound
Blaster Pro 2 (`sbtype = sbpro2`) was flipped due to a coding error. The
difference can be heard in games that toggle the filter, such as **Doom**, when
...
The OPL (AdLib) chip is now initialised with the official AdLib v1.51 driver's startup
values instead of the chip's native power-on state. 99.99% of games don't care
about this because they will fully overwrite the card with whatever state they
need, but this fixed the missing intro jingle in some Silmarils games.
...
- The `nosound` option now works better because it renders the audio and
  ...
  stalled). Additionally, now you can capture the audio to a WAV file even in
  "no sound" mode.


There is a problem with CD audio support under Windows if using a physical USB
CD drive (SATA drives work fine), the CD audio does not play at all.

### Other audio improvements

- Now the Gravis UltraSound and Sound Blaster can coexist when you run
  `ULTRINIT.EXE` to play MIDI music in supported games via GUS patches.

- Turn off MIDI delay emulation for the MT-32. This fixes the intro tune in
  **Bumpy's Arcade Fantasy**, and probably other game startup issues too. 

- Use 16-bit integer rendering mode for the MT-32 which is more accurate to
  the real hardware and a lot faster. Also use "nice" partials mode and render
  at libmt32's 48 kHz "analog emulation" rate for improved authenticity.

- Map the **Capture MIDI action** to ++ctrl+alt+f6++ by default
  (++cmd+opt+f6++ on macOS).

- CD Audio from physical CD-ROMs is threaded now, eliminating latency spikes
  in the main thread.

- Windows CD Audio now goes through the DOSBox mixer (same as on Linux).

- No need to use clunky `-usecd` and `-listcd` mount parameters anymore (these
  have been removed). The underlying device is detected automatically from the
  mount point and falls back to "fake CD-ROM" directory mount if a test ioctl
  doesn't work.
...
- General keyboard handling accuracy improvements by incorporating
  reverse-engineered real BIOS keyboard routines into the emulator.
...
### Windows 3.1x file locking support
...
- Improve redirection handling in the DOS shell. This helps with a few game
  installers (e.g., the running the **B-Wing Add-On** installer for **X-Wing**
...
Pentium MMX emulation to run late-90s demoscene productions and the odd game
that has MMX-specific enhancements (e.g., **Extreme Assault** and **Z.A.R.**).
And yeah, now you can use the MMX-only real-time resonant filters in Impulse
...
running (even if it not optimally). The "max cycles" setting, on the other
hand, is quite problematic for a couple of reasons:
...
- The `max` setting is not transferable across setups as it depends on the
  ...
  computer, or on yours a few years later when you get your next CPU upgrade.
  The `max N%` cycles settings (e.g., `max 70%`) are plagued by the same
  problem.
...
- Some DOS programs can _really_ freak out when the emulated CPU is being
...
We're ready for the ARM revolution :sunglasses: — we started providing
