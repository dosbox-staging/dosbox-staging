...
If you're upgrading from an earlier DOSBox Staging version, please follow the
[upgrade instructions](#how-to-upgrade) below.

If you're a first-time user (i.e., you've never used DOSBox Staging before, or
you're migrating from some other DOSBox variants), please follow the [Getting
...
Alternatively, run DOSBox Staging with the `--printconf` option and it will
...
This method will work 90% of the time, but certain settings just cannot be
automatically migrated in some cases. Look for deprecation warnings in the
...
new default primary config on first launch, then reapply your old settings
manually. This is very simple to do: after backing up your existing primary
