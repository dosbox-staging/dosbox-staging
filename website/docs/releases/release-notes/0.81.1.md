...
This maintenance release addresses the following regressions introduced in
0.81.0:
...
- Fix the DOSBox Staging window gaining focus on emulated video mode changes.
...
- Fix various VGA double scanning related edge cases for demoscene productions.

- Improve support for multiple joysticks/game controllers (the mapper
  could behave erratically if you had more than a single controller connected).

- Emulate a memory expanded PCjr machine more faithfully. This improves game
  compatibilty, e.g., **Space Quest** (v1.0x and v2.2) and **King's Quest**
  (1986 PCjr DOS version) now work instead of hanging the emulator.

- Fixed crash when exiting from fullscreen mode on macOS and Linux.


Lastly, Spanish and Brazilian Portugese translations have been added and most
...
- The SVGA/VESA double scanning criteria has been refined which fixes the
...
- Support "unorthodox" scanline repeats on VGA in forced single scanning mode
  (e.g., with `glshader = sharp` or `glshader = crt-auto-arcade`). This increases compatibility with demoscene productions;
  for example, [Show by Majic 12](https://www.pouet.net/prod.php?which=1158)
  sets up a custom 320&times;64 mode then uses scanline-quintupling to repeat
  every line 5 times in the zoom-rotator part in the intro. The bug did not
  manifest when double scanning was enabled (e.g., with `crt-auto` on
  larger than 1080p host resolutions).
...
vgaonly`). A minority of games, however, has regressed by this and would crash
at startup.

As a workaround until the accuracy of our VGA emulation is improved, we're
introducing the `vga_render_per_scanline` setting in the `[dosbox]`
section. The setting is enabled by default, but you can disable it to get the
legacy rendering behaviour back.
...
Note that these games are also CPU speed sensitive --- see the [issue
...
Fixed a regression where the Tandy version of **Impossible Mission II**
could not be started as the game just crashed. This fix should restore our
general compatibility with non-standard Tandy graphics modes.
...
A few games use the seek function of the CD-ROM but this was not implemented
in our emulated MSCDEX driver previously. This resulted in CD Audio not always
being played in certain games.

The new seek implementation fixes CD Audio related issues in at least the
following titles:
...
- Fixed various problems in the mapper when using more than a single game controller
  ...
  could randomly target the mappings of the second joystick and vice versa).
...
- Fixed obscure regression where the `timed = on` joystick setting messed up the
  available free memory reported by the FreeDOS version of `MEM.EXE`.
...
- Fixed a regression that resulted in the batch file installer of **Alpha
  Storm** fail.

- Introduced a new `pcjr_memory_config` setting in the DOSBox section,
  defaulting to `expanded`, which improves PCjr game compatibility.
  **Space Quest** (v1.0x and v2.2) and **King's Quest** (1986 PCjr DOS
  version) now work fine --- these games resulted in the emulator hanging
  before. The old behaviour can be restored with
  the `pcjr_memory_config = standard` setting (this is required for a handful
  of titles, such as the DOS conversions of the **Jumpman** and **Troll** PC
  Booter games).
...
- Spanish and Brazilian Portugese translations have been added.
...
## Project maintenance related changes
...
- Fixed DOSBox Staging crashing when exiting from fullscreen mode on macOS and Linux.

- Attempting to change unchangeable settings via the `CONFIG` command at
  runtime is no longer silently accepted (without anything happening) but an
