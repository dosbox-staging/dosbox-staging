...
command to manage keyboard layouts and code pages at runtime (run `KEYB /?`
for details), or the `CHCP` command to switch just the code page while keeping
the current keyboard layout (run `CHCP /?` for details).
