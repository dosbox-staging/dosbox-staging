...
// the logs, to the DOS console, or to the OSD). A single notification can
...
//   config settings)
//
// Positive feedback gives reassurance that an user-initiated action succeeded
...
// set an invalid config setting values via a DOS console command).
//
...
enum class Level {
	// Notifications related to single, discrete events should use the `Info`
	// level. This is mostly intended for user feedback (e.g., switching to a
	// different Sound Blaster model, muting the audio, changing the
	// monochrome palette, switching to the next mounted ISO image, etc.)
	Info,

	// Notications related to continuously adjustable settings should prefer
	// the `ContinuousInfo` over `Info` (e.g., continuously changing the
	// cycles setting, the composite video parameters, calibrating the
	// joystick, adjusting the horizontal/vertical video stretch factors,
	// etc.)
	//
	// The notification system is responsible for performing de-duplication or
	// throttling of the messages before presenting them to the user; the
	// caller need not to worry about that.
	ContinuousInfo,

	// The `Warning` notification level should be generally used to warn about
	// invalid configuration change attempts initiated by the user from the
	...
	// hiding or graying out invalid options.
	//
	...
	// The `Error` level is reserved for situations when something went really
	// wrong. This is usually a runtime condition that negatively affects the
	// functioning of the emulator (e.g., a dynamically linked library cannot
	// be loaded, an MT-32 model could not be initialised due to an invalid
	// ROM file, etc.)
	//
...
void display_message([[maybe_unused]] const Level level, const Source source,
                     ...
                     const std::string& message_name, const Args&... args) noexcept
{
	auto format_log_message = [&]() -> std::string {
		const auto format = strip_ansi_markup(MSG_GetEnglishRaw(message_name));
...
{
	display_message(Level::Info, source, topic, message_name, args...);
}

/*
 * Display a informational message in response to the user changing a
 * contiuously adjustable settin.
 *
...
void NOTIFY_DisplayContinuousInfoMessage(const Source source,
                                         ...
                                         const Args&... args) noexcept
{
	display_message(Level::ContinuousInfo, source, topic, message_name, args...);
}
...
void NOTIFY_DisplayWarning(const Source source, const std::string_view topic,
                           const std::string& message_name, const Args&... args) noexcept
{
	display_message(Level::Warning, source, topic, message_name, args...);
}
...
void NOTIFY_DisplayError(const Source source, const std::string_view topic,
                         const std::string& message_name, const Args&... args) noexcept
{
	display_message(Level::Error, source, topic, message_name, args...);
}
